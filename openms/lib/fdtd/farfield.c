...
static char name_Ex_real[20], name_Ex_imag[20], name_Ey_real[20],
    name_Ey_imag[20];
static char name_Hx_real[20], name_Hx_imag[20], name_Hy_real[20],
    name_Hy_imag[20];
static char FFT_E_int2[20], FFT_E_log[20], FFT_H_int2[20], FFT_H_log[20];
static char radiation_tot[20], radiation_Et[20], radiation_Ep[20],
    radiation_Ex[20], radiation_Ey[20];
...
void set_global_variable(int NROW, float OMEGA, float Nfree, int *POLROT,
                         int *dir, float *Eta, float *k);

//-----------------------------------------------------------
// reading (NROW)x(NROW) source data
// then, store them in efieldx_real[][], efieldx_imag[][] ...
//-----------------------------------------------------------
void reading_source_Ex(int row, int col, float **efieldx_real,
                       float **efieldx_imag);
void reading_source_Ey(int row, int col, float **efieldy_real,
                       float **efieldy_imag);
void reading_source_Hx(int row, int col, float **hfieldx_real,
                       float **hfieldx_imag);
void reading_source_Hy(int row, int col, float **hfieldy_real,
                       float **hfieldy_imag);

//------------------------------------------------------------
// Fourier transforming Ex, Ey, Hx, Hy to get Lx, Ly, Nx, Ny
//------------------------------------------------------------
void fourier_transform_Ly_Ex(int row, int col, int dir, float **Ly_real,
                             float **Ly_imag, float **efieldx_real,
                             float **efieldx_imag);
void fourier_transform_Lx_Ey(int row, int col, int dir, float **Lx_real,
                             float **Lx_imag, float **efieldy_real,
                             float **efieldy_imag);
void fourier_transform_Ny_Hx(int row, int col, int dir, float **Ny_real,
                             float **Ny_imag, float **hfieldx_real,
                             float **hfieldx_imag);
void fourier_transform_Nx_Hy(int row, int col, int dir, float **Nx_real,
                             float **Nx_imag, float **hfieldy_real,
                             float **hfieldy_imag);
...
//------------------------------------------------------------------------
void Ly_data_shrink(int row, int col, float **Ly_real, float **Ly_imag,
                    float **Ly_s_real, float **Ly_s_imag, float Nfree, float k);
void Lx_data_shrink(int row, int col, float **Lx_real, float **Lx_imag,
                    float **Lx_s_real, float **Lx_s_imag, float Nfree, float k);
void Nx_data_shrink(int row, int col, float **Nx_real, float **Nx_imag,
                    float **Nx_s_real, float **Nx_s_imag, float Nfree, float k);
void Ny_data_shrink(int row, int col, float **Ny_real, float **Ny_imag,
                    float **Ny_s_real, float **Ny_s_imag, float Nfree, float k);

//----------------------------------------------------------
// Calculate Nt,Np,Lt,Lp, ex_real,ex_imag, ey_real,ey_imag
//----------------------------------------------------------
void set_radiation_variable(
    float Nfree, float k, float Eta, float **Nx_s_real, float **Nx_s_imag,
    float **Ny_s_real, float **Ny_s_imag, float **Lx_s_real, float **Lx_s_imag,
    float **Ly_s_real, float **Ly_s_imag, float **Nt_s_real, float **Nt_s_imag,
    float **Np_s_real, float **Np_s_imag, float **Lt_s_real, float **Lt_s_imag,
    float **Lp_s_real, float **Lp_s_imag, float **cosp, float **sinp,
    float **cost, float **ex_real, float **ex_imag, float **ey_real,
    float **ey_imag);

//------------------------------------------------
// Calculate radint, etheta, ephi, eintx, einty
//------------------------------------------------
void calc_radiation(float Nfree, float k, float Eta, float **Nt_s_real,
                    float **Nt_s_imag, float **Np_s_real, float **Np_s_imag,
                    float **Lt_s_real, float **Lt_s_imag, float **Lp_s_real,
                    float **Lp_s_imag, float **cosp, float **sinp, float **cost,
                    float **ex_real, float **ex_imag, float **ey_real,
                    float **ey_imag, float **radint, float **etheta,
                    float **ephi, float **eintx, float **einty, float *P_tot,
                    float *P_the, float *P_phi);

//------------------------------------------------
// Print radint, etheta, ephi, eintx, einty
//------------------------------------------------
void print_radiation(float Nfree, float k, float Eta, float **radint,
                     float **etheta, float **ephi, float **eintx, float **einty,
                     float *P_tot, float *P_the, float *P_phi);

void FT_data_writing(float **Nx_s_real, float **Nx_s_imag, float **Ny_s_real,
                     float **Ny_s_imag, float **Lx_s_real, float **Lx_s_imag,
                     float **Ly_s_real, float **Ly_s_imag, int row_s, int col_s,
                     float Nfree, float k);
  ...
  for (j = 1; j <= jsize - 2; j++) {
    for (i = 1; i <= isize - 2; i++) {
      for (mm = 0; mm < SpecN; mm++) {
        Ex_cos[i][j][mm] +=
            grid_value("Ex", i, j, k) *
            cos(2 * pi * OMEGA[mm] * t / S_factor / ds_x / lattice_x);
        Ex_sin[i][j][mm] +=
            grid_value("Ex", i, j, k) *
            sin(2 * pi * OMEGA[mm] * t / S_factor / ds_x / lattice_x);
        Ey_cos[i][j][mm] +=
            grid_value("Ey", i, j, k) *
            cos(2 * pi * OMEGA[mm] * t / S_factor / ds_x / lattice_x);
        Ey_sin[i][j][mm] +=
...
void far_field_FFT(int NROW, float NA, float Nfree, float *OMEGA, int mm) {
  ...
  //-----------------------------------------------------------------------------------
  fourier_transform_Ly_Ex(row, col, dir, Ly_real, Ly_imag, efieldx_real,
                          efieldx_imag);
  //-----------------------------------------------------------------------------------
  for (i = 0; i < col; i++) {
    free(efieldx_real[i]);
  ...
  //-----------------------------------------------------------------------------------
  fourier_transform_Lx_Ey(row, col, dir, Lx_real, Lx_imag, efieldy_real,
                          efieldy_imag);
  //-----------------------------------------------------------------------------------
  for (i = 0; i < col; i++) {
    free(efieldy_real[i]);
  ...
  //-----------------------------------------------------------------------------------
  fourier_transform_Ny_Hx(row, col, dir, Ny_real, Ny_imag, hfieldx_real,
                          hfieldx_imag);
  //-----------------------------------------------------------------------------------
  for (i = 0; i < col; i++) {
    free(hfieldx_real[i]);
  ...
  //-----------------------------------------------------------------------------------
  fourier_transform_Nx_Hy(row, col, dir, Nx_real, Nx_imag, hfieldy_real,
                          hfieldy_imag);
  //-----------------------------------------------------------------------------------
  for (i = 0; i < col; i++) {
    free(hfieldy_real[i]);
  ...
  FT_data_writing(Nx_s_real, Nx_s_imag, Ny_s_real, Ny_s_imag, Lx_s_real,
                  Lx_s_imag, Ly_s_real, Ly_s_imag, row_s, col_s, Nfree, k);

  //--------------------------------------------------------------------------------------------------
  set_radiation_variable(Nfree, k, Eta, Nx_s_real, Nx_s_imag, Ny_s_real,
                         Ny_s_imag, Lx_s_real, Lx_s_imag, Ly_s_real, Ly_s_imag,
                         Nt_s_real, Nt_s_imag, Np_s_real, Np_s_imag, Lt_s_real,
                         Lt_s_imag, Lp_s_real, Lp_s_imag, cosp, sinp, cost,
  ...
  //--------------------------------------------------------------------------------------------------
  calc_radiation(Nfree, k, Eta, Nt_s_real, Nt_s_imag, Np_s_real, Np_s_imag,
                 Lt_s_real, Lt_s_imag, Lp_s_real, Lp_s_imag, cosp, sinp, cost,
                 ex_real, ex_imag, ey_real, ey_imag, radint, etheta, ephi,
                 eintx, einty, &P_tot, &P_the, &P_phi);
  //--------------------------------------------------------------------------------------------------
  for (i = 0; i < col_s + 1; i++) {
    free(Nt_s_real[i]);
    free(Nt_s_imag[i]);
  ...
  //-----------------------------------------------------------------------------------------
  print_radiation(Nfree, k, Eta, radint, etheta, ephi, eintx, einty, &P_tot,
                  &P_the, &P_phi);
  //-----------------------------------------------------------------------------------------
...
void Nx_data_shrink(int row, int col, float **Nx_real, float **Nx_imag,
                    float **Nx_s_real, float **Nx_s_imag, float Nfree,
                    float k) {
  int i, j;
  int half_row, half_col;
  ...
  for (j = (half_row) + (half_row_s); j >= (half_row) - (half_row_s); j--) {
    for (i = (half_col) - (half_col_s); i <= (half_col) + (half_col_s); i++) {
      Nx_s_real[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Nx_real[i][j];
      Nx_s_imag[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Nx_imag[i][j];
    }
  }
}
...
void Ny_data_shrink(int row, int col, float **Ny_real, float **Ny_imag,
                    float **Ny_s_real, float **Ny_s_imag, float Nfree,
                    float k) {
  int i, j;
  int half_row, half_col;
  int row_s, col_s; // shrinked row & col
  ...
  for (j = (half_row) + (half_row_s); j >= (half_row) - (half_row_s); j--) {
    for (i = (half_col) - (half_col_s); i <= (half_col) + (half_col_s); i++) {
      Ny_s_real[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Ny_real[i][j];
      Ny_s_imag[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Ny_imag[i][j];
    }
  }
}
...
void Lx_data_shrink(int row, int col, float **Lx_real, float **Lx_imag,
                    float **Lx_s_real, float **Lx_s_imag, float Nfree,
                    float k) {
  int i, j;
  int half_row, half_col;
  int row_s, col_s; // shrinked row & col
  ...
  for (j = (half_row) + (half_row_s); j >= (half_row) - (half_row_s); j--) {
    for (i = (half_col) - (half_col_s); i <= (half_col) + (half_col_s); i++) {
      Lx_s_real[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Lx_real[i][j];
      Lx_s_imag[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Lx_imag[i][j];
    }
  }
}
...
void Ly_data_shrink(int row, int col, float **Ly_real, float **Ly_imag,
                    float **Ly_s_real, float **Ly_s_imag, float Nfree,
                    float k) {
  int i, j;
  int half_row, half_col;
  int row_s, col_s; // shrinked row & col
  ...
  for (j = (half_row) + (half_row_s); j >= (half_row) - (half_row_s); j--) {
    for (i = (half_col) - (half_col_s); i <= (half_col) + (half_col_s); i++) {
      Ly_s_real[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Ly_real[i][j];
      Ly_s_imag[i - (half_col) + (half_col_s)][j - (half_row) + (half_row_s)] =
          Ly_imag[i][j];
    }
  }
}
...
void FT_data_writing(float **Nx_s_real, float **Nx_s_imag, float **Ny_s_real,
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i <= (col_s); i++) {
      temp = Lx_s_real[i][j] * Lx_s_real[i][j] +
             Ly_s_real[i][j] * Ly_s_real[i][j] +
             Lx_s_imag[i][j] * Lx_s_imag[i][j] +
             Ly_s_imag[i][j] * Ly_s_imag[i][j];
      if (temp > FFT_Emax)
        FFT_Emax = temp;
    }
  }
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i <= (col_s); i++) {
      fprintf(stream, "%g\t",
              log10((Lx_s_real[i][j] * Lx_s_real[i][j] +
                     Ly_s_real[i][j] * Ly_s_real[i][j] +
                     Lx_s_imag[i][j] * Lx_s_imag[i][j] +
                     Ly_s_imag[i][j] * Ly_s_imag[i][j]) /
                    FFT_Emax));
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i <= (col_s); i++) {
      temp = Nx_s_real[i][j] * Nx_s_real[i][j] +
             Ny_s_real[i][j] * Ny_s_real[i][j] +
             Nx_s_imag[i][j] * Nx_s_imag[i][j] +
             Ny_s_imag[i][j] * Ny_s_imag[i][j];
      if (temp > FFT_Hmax)
        FFT_Hmax = temp;
    }
  }
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i <= (col_s); i++) {
      fprintf(stream, "%g\t",
              log10((Nx_s_real[i][j] * Nx_s_real[i][j] +
                     Ny_s_real[i][j] * Ny_s_real[i][j] +
                     Nx_s_imag[i][j] * Nx_s_imag[i][j] +
                     Ny_s_imag[i][j] * Ny_s_imag[i][j]) /
                    FFT_Hmax));
...
void set_radiation_variable(
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      if ((i - half_col_s) * (i - half_col_s) +
              (j - half_row_s) * (j - half_row_s) <
          Nfree * Nfree * k * k) {
        cosp[i][j] =
            (i - half_col_s) / sqrt((i - half_col_s) * (i - half_col_s) +
                                    (j - half_row_s) * (j - half_row_s));
        sinp[i][j] =
            (j - half_row_s) / sqrt((i - half_col_s) * (i - half_col_s) +
                                    (j - half_row_s) * (j - half_row_s));
        cost[i][j] = sqrt(1 - ((i - half_col_s) * (i - half_col_s) +
                               (j - half_row_s) * (j - half_row_s)) /
                                  (Nfree * Nfree * k * k));
      } else {
        cosp[i][j] = 0.0;
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      Nt_s_real[i][j] =
          (Nx_s_real[i][j] * cosp[i][j] + Ny_s_real[i][j] * sinp[i][j]) *
          cost[i][j];
      Nt_s_imag[i][j] =
          (Nx_s_imag[i][j] * cosp[i][j] + Ny_s_imag[i][j] * sinp[i][j]) *
          cost[i][j];
      Np_s_real[i][j] =
          (-Nx_s_real[i][j] * sinp[i][j] + Ny_s_real[i][j] * cosp[i][j]);
      Np_s_imag[i][j] =
          (-Nx_s_imag[i][j] * sinp[i][j] + Ny_s_imag[i][j] * cosp[i][j]);
      Lt_s_real[i][j] =
          (Lx_s_real[i][j] * cosp[i][j] + Ly_s_real[i][j] * sinp[i][j]) *
          cost[i][j];
      Lt_s_imag[i][j] =
          (Lx_s_imag[i][j] * cosp[i][j] + Ly_s_imag[i][j] * sinp[i][j]) *
          cost[i][j];
      Lp_s_real[i][j] =
          (-Lx_s_real[i][j] * sinp[i][j] + Ly_s_real[i][j] * cosp[i][j]);
      Lp_s_imag[i][j] =
          (-Lx_s_imag[i][j] * sinp[i][j] + Ly_s_imag[i][j] * cosp[i][j]);
    }
  }
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      ex_real[i][j] =
          -(Np_s_real[i][j] - Lt_s_real[i][j] / Eta) * sinp[i][j] +
          (Nt_s_real[i][j] + Lp_s_real[i][j] / 377) * cost[i][j] * cosp[i][j];
      ex_imag[i][j] =
          -(Np_s_imag[i][j] - Lt_s_imag[i][j] / Eta) * sinp[i][j] +
          (Nt_s_imag[i][j] + Lp_s_imag[i][j] / 377) * cost[i][j] * cosp[i][j];
      ey_real[i][j] =
          (Np_s_real[i][j] - Lt_s_real[i][j] / Eta) * cosp[i][j] +
          (Nt_s_real[i][j] + Lp_s_real[i][j] / Eta) * cost[i][j] * sinp[i][j];
      ey_imag[i][j] =
          (Np_s_imag[i][j] - Lt_s_imag[i][j] / Eta) * cosp[i][j] +
          (Nt_s_imag[i][j] + Lp_s_imag[i][j] / Eta) * cost[i][j] * sinp[i][j];
    }
  }
  printf("Ex, Ey ok!......\n");
}
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      if ((i - half_col_s) * (i - half_col_s) +
              (j - half_row_s) * (j - half_row_s) <
          Nfree * Nfree * k * k)
        radint[i][j] = ((Nt_s_real[i][j] + Lp_s_real[i][j] / Eta) *
                            (Nt_s_real[i][j] + Lp_s_real[i][j] / Eta) +
                        (Nt_s_imag[i][j] + Lp_s_imag[i][j] / Eta) *
                            (Nt_s_imag[i][j] + Lp_s_imag[i][j] / Eta) +
                        (Np_s_real[i][j] - Lt_s_real[i][j] / Eta) *
                            (Np_s_real[i][j] - Lt_s_real[i][j] / Eta) +
                        (Np_s_imag[i][j] - Lt_s_imag[i][j] / Eta) *
                            (Np_s_imag[i][j] - Lt_s_imag[i][j] / Eta));
      else
        radint[i][j] = 0.0;
    }
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      if ((i - half_col_s) * (i - half_col_s) +
              (j - half_row_s) * (j - half_row_s) <
          Nfree * Nfree * k * k)
        etheta[i][j] = ((Nt_s_real[i][j] + Lp_s_real[i][j] / Eta) *
                            (Nt_s_real[i][j] + Lp_s_real[i][j] / Eta) +
                        (Nt_s_imag[i][j] + Lp_s_imag[i][j] / Eta) *
                            (Nt_s_imag[i][j] + Lp_s_imag[i][j] / Eta));
      else
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      if ((i - half_col_s) * (i - half_col_s) +
              (j - half_row_s) * (j - half_row_s) <
          Nfree * Nfree * k * k)
        ephi[i][j] = ((Np_s_real[i][j] - Lt_s_real[i][j] / Eta) *
                          (Np_s_real[i][j] - Lt_s_real[i][j] / Eta) +
                      (Np_s_imag[i][j] - Lt_s_imag[i][j] / Eta) *
                          (Np_s_imag[i][j] - Lt_s_imag[i][j] / Eta));
      else
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      if ((i - half_col_s) * (i - half_col_s) +
          ...
          Nfree * Nfree * k * k)
        eintx[i][j] =
            (ex_real[i][j] * ex_real[i][j] + ex_imag[i][j] * ex_imag[i][j]);
      else
        eintx[i][j] = 0;
    }
  }
  ...
  for (j = row_s; j >= 0; j--) {
    for (i = 0; i < (col_s + 1); i++) {
      if ((i - half_col_s) * (i - half_col_s) +
          ...
          Nfree * Nfree * k * k)
        einty[i][j] =
            (ey_real[i][j] * ey_real[i][j] + ey_imag[i][j] * ey_imag[i][j]);
      else
        einty[i][j] = 0;
    }
  }
...
void print_radiation(float Nfree, float k, float Eta, float **radint,
  ...
  for (j = half_row_s + (int)(Nfree * k) - 1;
       j >= half_row_s - (int)(Nfree * k) + 1; j--) {
    for (i = half_col_s - (int)(Nfree * k) + 1;
         i <= half_col_s + (int)(Nfree * k) - 1; i++) {
...
void calc_polar(float NA, float Nfree)
{
        ...
        for(l=0; l<POLROT; l++)
        {
                ...
                for(j=row; j>=0; j--)
                {
                        ...
                        {
                                if(
(i-half_col)*(i-half_col)+(j-half_row)*(j-half_row) < NA*NA*Nfree*Nfree*k*k)
                                {
                                        Eu_real[i][j] =
ex_real[i][j]*cos(2*pi*l/POLROT) + ey_real[i][j]*sin(2*pi*l/POLROT);
                                        Eu_imag[i][j] =
ex_imag[i][j]*cos(2*pi*l/POLROT) + ey_imag[i][j]*sin(2*pi*l/POLROT);
                                        Eu_int[i][j] =
Eu_real[i][j]*Eu_real[i][j] + Eu_imag[i][j]*Eu_imag[i][j]; sum_Eu = sum_Eu +
Eu_int[i][j]*sqrt(1-cost[i][j]*cost[i][j])/FFT_Emax;
                                }
                        }
                }
