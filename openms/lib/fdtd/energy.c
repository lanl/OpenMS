...
void total_E_energy() {
  ...
  for (i = 1 + pmlil + 0.2 * lattice_x;
       i <= isize - 2 - pmlir - 0.2 * lattice_x; i++)
    for (k = ksize - 2 - pmlkr - 0.2 * lattice_z;
         k >= 1 + pmlkl + 0.2 * lattice_z; k--)
      for (j = 1 + pmljl + 0.2 * lattice_y;
           j <= jsize - 2 - pmljr - 0.2 * lattice_y; j++)
...
void total_E_energy_block(float centerx, float centery, float centerz,
                          float size1, float size2, float size3) {
  FILE *stream_Eb;
  int i, j, k;
  float Eb = 0;
  int x_n, x_p, y_n, y_p, z_n,
      z_p; // n:- p:+, position of the sides of the block
...
void total_E_energy2_block(float centerx, float centery, float centerz,
                           float size1, float size2,
                           float size3) { // for calculating kinetic energy of
                                          // electrons (from ver. 8.78)
  FILE *stream_E2b;
  int i, j, k;
  float Eb = 0;
  int x_n, x_p, y_n, y_p, z_n,
...
void total_E_energy3_block(float centerx, float centery, float centerz,
                           float size1, float size2,
                           float size3) { // for calculating potential energy
                                          // of electrons (from ver. 8.78)
  FILE *stream_E3b;
  int i, j, k;
  float Eb = 0;
  int x_n, x_p, y_n, y_p, z_n,
...
void total_E_energy_thin_block_z(float centerx, float centery, float centerz,
                                 float size1, float size2, float size3,
                                 float eps_L, float eps_H, char *name) {
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /// This calculation assumes very very thin z-size (=size3), so we takes an
  /// arithmatic average of U(z_n) and U(z_p)
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  FILE *file;
  int i, j, k;
  float Ebt = 0;
  ...
  for (i = x_n; i <= x_p; i++)
    for (j = y_n; j <= y_p; j++)
      if (eps(i, j, z_n) >= eps_L && eps(i, j, z_n) <= eps_H &&
          eps(i, j, z_p) >= eps_L && eps(i, j, z_p) <= eps_H)
        Ebt = Ebt + 0.5 * ds_x * ds_y * (size3 * ds_x * lattice_x) *
                        (grid_value("E_Energy", i, j, z_n) +
                         grid_value("E_Energy", i, j, z_p));
...
void max_E_Energy_detector(float centerx, float centery, float centerz,
                           float size1, float size2, float size3) {
  FILE *stream_max;
  int i, j, k;
  float max_val = -12345;
  ...
  for (i = 1 + pmlil + 0.2 * lattice_x;
    ...
    for (k = ksize - 2 - pmlkr - 0.2 * lattice_z;
      ...
      for (j = 1 + pmljl + 0.2 * lattice_y;
           j <= jsize - 2 - pmljr - 0.2 * lattice_y; j++)
        eEE_uHH =
            eEE_uHH + ds_x * ds_y * ds_nz[k] * grid_value("EM_Energy", i, j, k);
...
void total_EM_energy_block(float centerx, float centery, float centerz,
                           float size1, float size2, float size3) {
  FILE *stream_EMb;
  int i, j, k;
  float EMb = 0;
...
void Drude_energy_loss_in_block2(float centerx, float centery, float centerz,
                                 float size1, float size2, float size3,
                                 float WC, float lattice_n) {
  FILE *stream_Eloss;
  int i, j, k;
  double Eloss = 0.0;
  double Im_eps;
  ...
  //
  //	Im{epsilon(w)} = momega[i][j][k]^2 * mgamma[i][j][k] / ( w^3 +
  // w*mgamma[i][j][k]^2 )
  /////////////////////////////////////////////////////////////////////////////////////////////
  ...
  wr = WC * 2 * 3.1415926 * light_speed /
       lattice_n; // into actual angular frequency in the unit of Hz

  temp_fac = (ds_x * lattice_x) /
             lattice_n; // to be used to reconvert the reduced wp and gamma

  for (i = x_n; i <= x_p; i++)
    for (j = y_n; j <= y_p; j++)
      for (k = z_n; k <= z_p; k++) {
        Im_eps = temp_fac * temp_fac * temp_fac * momega[i][j][k] *
                 momega[i][j][k] * mgamma[i][j][k] /
                 (wr * wr * wr +
                  wr * mgamma[i][j][k] * mgamma[i][j][k] * temp_fac * temp_fac);
        Eloss = Eloss + 2 * w * eo * Im_eps * ds_x * ds_y * ds_nz[k] *
                            grid_value("E^2", i, j, k);
      }
...
void Drude_energy_loss_in_block(float centerx, float centery, float centerz,
                                float size1, float size2, float size3) {
  FILE *stream_Eloss;
  int i, j, k;
  double Eloss = 0.0;
...
void Poynting_total() {
  ...
  for (k = ksize - 2 - pmlkr - 0.2 * lattice_z;
    ...
    for (j = 1 + pmljl + 0.2 * lattice_y;
         j <= jsize - 2 - pmljr - 0.2 * lattice_y; j++) {
      sumxp += (ds_y * ds_nz[k]) *
               grid_value("Sx", isize - 2 - pmlir - 0.2 * lattice_x, j, k);
      sumxn +=
          (ds_y * ds_nz[k]) * grid_value("Sx", pmlil + 0.2 * lattice_x, j, k);
    }
  for (i = 1 + pmlil + 0.2 * lattice_x;
       i <= isize - 2 - pmlir - 0.2 * lattice_x; i++)
    for (k = ksize - 2 - pmlkr - 0.2 * lattice_z;
         k >= 1 + pmlkl + 0.2 * lattice_z; k--) {
      sumyp += (ds_nz[k] * ds_x) *
               grid_value("Sy", i, jsize - 2 - pmljl - 0.2 * lattice_y, k);
      sumyn +=
          (ds_nz[k] * ds_x) * grid_value("Sy", i, pmljl + 0.2 * lattice_y, k);
    }
  for (i = 1 + pmlil + 0.2 * lattice_x;
       i <= isize - 2 - pmlir - 0.2 * lattice_x; i++)
    for (j = 1 + pmljl + 0.2 * lattice_y;
         j <= jsize - 2 - pmljr - 0.2 * lattice_y; j++) {
      sumzp += (ds_x * ds_y) *
               grid_value("Sz", i, j, ksize - 2 - pmlkr - 0.2 * lattice_z);
...
float Poynting_half_sphere_point(char *component, float z0, float R,
  ...
  if (strcmp("r", component) == 0) {
    Pi = (ds_y * ds_nz[k]) * grid_value("Sx", xg, yg, zg) * sin(theta) *
             cos(phi) +
         (ds_nz[k] * ds_x) * grid_value("Sy", xg, yg, zg) * sin(theta) *
             sin(phi) +
         (ds_x * ds_y) * grid_value("Sz", xg, yg, zg) * cos(theta);
  } else if (strcmp("theta", component) == 0) {
    Pi = (ds_y * ds_nz[k]) * grid_value("Sx", xg, yg, zg) * cos(theta) *
             cos(phi) +
         (ds_nz[k] * ds_x) * grid_value("Sy", xg, yg, zg) * cos(theta) *
             sin(phi) -
...
void Poynting_block(float centerx, float centery, float centerz, float size1,
                    float size2, float size3) {
  int i, j, k;
  float sumxp = 0, sumxn = 0, sumyp = 0, sumyn = 0, sumzp = 0, sumzn = 0;
  int x_n, x_p, y_n, y_p, z_n,
      z_p; // n:- p:+, position of the sides of the block
...
void Poynting_side(float value, float zposition) {
  ...
  for (k = non_uniform_z_to_i(value + zposition);
       k >= non_uniform_z_to_i(-value + zposition); k--)
    for (j = 1 + pmljl + 0.2 * lattice_y;
         j <= jsize - 2 - pmljr - 0.2 * lattice_y; j++) {
      sumxp += (ds_y * ds_nz[k]) *
               grid_value("Sx", isize - 2 - pmlir - 0.2 * lattice_x, j, k);
  ...
  for (i = 1 + pmlil + 0.2 * lattice_x;
    ...
    for (k = non_uniform_z_to_i(value + zposition);
         k >= non_uniform_z_to_i(-value + zposition); k--) {
      sumyp += (ds_nz[k] * ds_x) *
               grid_value("Sy", i, jsize - 2 - pmljr - 0.2 * lattice_y, k);
      sumyn +=
          (ds_nz[k] * ds_x) * grid_value("Sy", i, pmljl + 0.2 * lattice_y, k);
    }
...
void print_energy() {
  ...
  fprintf(VQSQ,
          "VerticalLoss=%g\t HorizontalLoss=%g\t "
          "VerticalLoss/HorizontalLoss(K)=%g\n",
          Ev, Eh, Ev / Eh);
...
void Poynting_UpDown(float value, float zposition) {
  ...
  ///////// Lower Strip
  for (k = 1 + pmlkl + 0.2 * lattice_z;
       k <= non_uniform_z_to_i(-value + zposition); k++)
    for (j = 1 + pmljl + 0.2 * lattice_y;
         j <= jsize - 2 - pmljr - 0.2 * lattice_y; j++) {
      DnStripXp += (ds_y * ds_nz[k]) *
                   grid_value("Sx", isize - 2 - pmlir - 0.2 * lattice_x, j, k);
      DnStripXn +=
          (ds_y * ds_nz[k]) * grid_value("Sx", pmlil + 0.2 * lattice_x, j, k);
    }
  ...
  SumUpper =
      SumUpper + PlanZp + (UpStripXp - UpStripXn) + (UpStripYp - UpStripYn);
  SumLower =
      SumLower + (-PlanZn) + (DnStripXp - DnStripXn) + (DnStripYp - DnStripYn);
}
