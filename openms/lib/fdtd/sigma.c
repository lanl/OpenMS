...
float cpmlax(float a, float pmlbl, float pmlbr)
{
    ...
    if(use_periodic_x==0) // no use of periodic boundary condition
    {
        ...
        else if( a<isize-pmlbr-1) return 0.0;
        else
        {
            sigmae=sigmaCPML*pow((a-(isize-pmlbr-1.0))/pmlbr, m_pml);
            alphae=alphaCPML*pow((isize-1.0-a)/pmlbr,ma_pml);
...
float kappa_x(float a, float pmlbl, float pmlbr)
{
    ...
    if(use_periodic_x==0) // no use of periodic boundary condition
    {
        ...
        else kappae=1.0+(kappaCPML-1.0)*pow((a-(isize-pmlbr-1.0))/pmlbr,m_pml);
