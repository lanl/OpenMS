...
float grid_value_periodic_xy(char *component, int i, int j, int k, int h,
                             int v);

int find_k_projection(int i, int j, int k_range);

int nz_multiple(int k); // for non_uniform_grid multiplication, used in various
                        // out_put image functions
...
void out_epsilon(char *plane, float value, char *name) {
  ...
  if (strcmp("x", plane) == 0) {
    ...
    for (k = k_range; k >= 1; k--) {
      for (mz = 0; mz < nz_multiple(k);
           mz++) // for non_uniform_grid multiplication
      {
        for (j = 1; j <= j_range; j++) {
          if (meps(i, j, k) == 0.0)
...
void out_epsilon_periodic(char *plane, float value, char *name, int m_h,
                          int m_v) {
  FILE *file;
  int i, j, k;
  int v, h;
...
void out_plane_periodic(char *component, char *plane, float value,
                        char *lastname, int m_h, int m_v) {
  FILE *file;
  char name[20];
  int i, j, k;
  int h, v;
      ...
      for (k = k_range; k >= 1; k--) {
             ...
             mz++) // for non_uniform_grid multiplication
        {
          for (h = 0; h < m_h; h++) {
            for (j = 1; j <= j_range; j++)
              fprintf(file, "%g ",
                      grid_value_periodic_y(component, i, j, k, h));
          }
          fprintf(file, "\n");
        }
      }
      ...
      for (k = k_range; k >= 1; k--) {
             ...
             mz++) // for non_uniform_grid multiplication
        {
          for (h = 0; h < m_h; h++) {
            for (i = 1; i <= i_range; i++)
              fprintf(file, "%g ",
                      grid_value_periodic_x(component, i, j, k, h));
          }
          fprintf(file, "\n");
        }
      }
    ...
    for (v = 0; v < m_v; v++) {
      for (j = j_range; j >= 1; j--) {
        for (h = 0; h < m_h; h++) {
          for (i = 1; i <= i_range; i++)
            fprintf(file, "%g ",
                    grid_value_periodic_xy(component, i, j, k, h, v));
        }
        fprintf(file, "\n");
      }
    }
...
void out_plane_time_average(char *component, char *plane, float value,
                            long start, long end, float ***field_avg,
                            char *lastname) {
  FILE *file;
  char name[20];
  int i, j, k;
  ...
  if (strcmp("x", plane) == 0) {
    ...
    if (t > start && t <= end) {
      ...
      for (k = k_range; k >= 1; k--) {
        for (j = 1; j <= j_range; j++)
          (*field_avg)[j][k] =
              (*field_avg)[j][k] + grid_value(component, i, j, k);
      }
    }
    ...
    if (t == end) {
      ...
      for (k = k_range; k >= 1; k--) {
        ...
        {
          for (j = 1; j <= j_range; j++)
            fprintf(file, "%g ", (*field_avg)[j][k] / (float)(end - start));
          fprintf(file, "\n");
        }
      }
  ...
  if (strcmp("y", plane) == 0) {
    ...
    if (t > start && t <= end) {
      ...
      for (k = k_range; k >= 1; k--) {
        for (i = 1; i <= i_range; i++)
          (*field_avg)[k][i] =
              (*field_avg)[k][i] + grid_value(component, i, j, k);
      }
    }
    ...
    if (t == end) {
      ...
      for (k = k_range; k >= 1; k--) {
        ...
        {
          for (i = 1; i <= i_range; i++)
            fprintf(file, "%g ", (*field_avg)[k][i] / (float)(end - start));
          fprintf(file, "\n");
        }
      }
  ...
  if (strcmp("z", plane) == 0) {
    ...
    if (t > start && t <= end) {
      ...
      for (j = j_range; j >= 1; j--) {
        for (i = 1; i <= i_range; i++)
          (*field_avg)[i][j] =
              (*field_avg)[i][j] + grid_value(component, i, j, k);
      }
    }
...
void out_plane_time_average_projection(char *component, char *dirc, long start,
                                       long end, float ***field_avg,
                                       char *lastname, int k_shift) {
  FILE *file;
  char name[20];
  int i, j, k;
  int i_range, j_range, k_range;
...
void out_plane_projection(char *component, char *dirc, char *lastname,
                          int k_shift) {
  FILE *file;
  char name[20];
  int i, j, k;
...
void out_several_points(char *component, float zposition, float xside,
                        float yside, int pNx, int pNy, long ti, long tf,
                        char *name) {
  FILE *file;
  int i, j, k;
  ...
  if (ti <= t && t < tf) {
    ...
    for (m = 0; m <= pNx; m++) {
      for (n = 0; n <= pNy; n++) {
        i = floor(0.5 +
                  ((-0.5 * xside + m * (xside / pNx) + xcenter) * lattice_x));
        j = floor(0.5 +
                  ((-0.5 * yside + n * (yside / pNx) + ycenter) * lattice_y));
        fprintf(file, "%g\t", grid_value(component, i, j, k));
      }
    }
    fprintf(file, "\n");
...
void out_point(char *component, float x, float y, float z, long ti, long tf,
               char *name) {
  FILE *file;
  int i, j, k;
...
float grid_value(char *component, int i, int j, int k) {
  ...
  if (strcmp(component, "Hx") == 0)
    return hfieldx(i, j, k);
  if (strcmp(component, "Hy") == 0)
    return hfieldy(i, j, k);
  if (strcmp(component, "Hz") == 0)
    return hfieldz(i, j, k);
  if (strcmp(component, "Jx") == 0)
    return jfieldx(i, j, k);
  if (strcmp(component, "Jy") == 0)
    return jfieldy(i, j, k);
  if (strcmp(component, "Jz") == 0)
    return jfieldz(i, j, k);
  if (strcmp(component, "J.E") == 0)
    return efieldx(i, j, k) * jfieldx(i, j, k) +
           efieldy(i, j, k) * jfieldy(i, j, k) +
           efieldz(i, j, k) * jfieldz(i, j, k);
  if (strcmp(component, "E^2") == 0)
    return pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
           pow(efieldz(i, j, k), 2);
  if (strcmp(component, "H^2") == 0)
    return pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2) +
           pow(hfieldz(i, j, k), 2);
  if (strcmp(component, "Sx") == 0)
    return efieldy(i, j, k) * hfieldz(i, j, k) -
           efieldz(i, j, k) * hfieldy(i, j, k);
  if (strcmp(component, "Sy") == 0)
    return efieldz(i, j, k) * hfieldx(i, j, k) -
           efieldx(i, j, k) * hfieldz(i, j, k);
  if (strcmp(component, "Sz") == 0)
    return efieldx(i, j, k) * hfieldy(i, j, k) -
           efieldy(i, j, k) * hfieldx(i, j, k);
  if (strcmp(component, "LogE^2") == 0)
    return log10(pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
                 pow(efieldz(i, j, k), 2));
  if (strcmp(component, "LogH^2") == 0)
    return log10(pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2) +
                 pow(hfieldz(i, j, k), 2));

  if (strcmp(component, "EM_Energy") == 0 && meps(i, j, k) == 0.0)
    return 0.5 * eo * eps(i, j, k) *
               (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
                pow(efieldz(i, j, k), 2)) +
           0.5 * uo * ups *
               (pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2) +
                pow(hfieldz(i, j, k), 2));
  if (strcmp(component, "EM_Energy") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m(i, j, k) *
               (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
                pow(efieldz(i, j, k), 2)) +
           0.5 * uo * ups *
               (pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2) +
                pow(hfieldz(i, j, k), 2));
  if (strcmp(component, "EM_Energy2") == 0 && meps(i, j, k) == 0.0)
    return 0.0;
  if (strcmp(component, "EM_Energy2") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m2(i, j, k) *
               (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
                pow(efieldz(i, j, k), 2)) +
           0.5 * uo * ups *
               (pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2) +
                pow(hfieldz(i, j, k), 2));
  if (strcmp(component, "EM_Energy3") == 0 && meps(i, j, k) == 0.0)
    return 0.0;
  if (strcmp(component, "EM_Energy3") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m3(i, j, k) *
               (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
                pow(efieldz(i, j, k), 2)) +
           0.5 * uo * ups *
               (pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2) +
  ...
  if (strcmp(component, "E_Energy") == 0 && meps(i, j, k) == 0.0)
    return 0.5 * eo * eps(i, j, k) *
           (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
            pow(efieldz(i, j, k), 2));
  if (strcmp(component, "E_Energy") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m(i, j, k) *
           (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
            pow(efieldz(i, j, k), 2));
  if (strcmp(component, "E_Energy2") == 0 && meps(i, j, k) == 0.0)
    return 0.0;
  if (strcmp(component, "E_Energy2") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m2(i, j, k) *
           (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
            pow(efieldz(i, j, k), 2));
  if (strcmp(component, "E_Energy3") == 0 && meps(i, j, k) == 0.0)
    return 0.0;
  if (strcmp(component, "E_Energy3") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m3(i, j, k) *
           (pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2) +
  ...
  if (strcmp(component, "Ex2Ey2") == 0)
    return pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2);
  if (strcmp(component, "Ey2Ez2") == 0)
    return pow(efieldy(i, j, k), 2) + pow(efieldz(i, j, k), 2);
  if (strcmp(component, "Ez2Ex2") == 0)
    return pow(efieldz(i, j, k), 2) + pow(efieldx(i, j, k), 2);
  if (strcmp(component, "LogEx2Ey2") == 0)
    return log10(pow(efieldx(i, j, k), 2) + pow(efieldy(i, j, k), 2));
  if (strcmp(component, "LogEy2Ez2") == 0)
    return log10(pow(efieldy(i, j, k), 2) + pow(efieldz(i, j, k), 2));
  if (strcmp(component, "LogEz2Ex2") == 0)
    return log10(pow(efieldz(i, j, k), 2) + pow(efieldx(i, j, k), 2));
  if (strcmp(component, "Hx2Hy2") == 0)
    return pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2);
  if (strcmp(component, "Hy2Hz2") == 0)
    return pow(hfieldy(i, j, k), 2) + pow(hfieldz(i, j, k), 2);
  if (strcmp(component, "Hz2Hx2") == 0)
    return pow(hfieldz(i, j, k), 2) + pow(hfieldx(i, j, k), 2);
  if (strcmp(component, "LogHx2Hy2") == 0)
    return log10(pow(hfieldx(i, j, k), 2) + pow(hfieldy(i, j, k), 2));
  if (strcmp(component, "LogHy2Hz2") == 0)
    return log10(pow(hfieldy(i, j, k), 2) + pow(hfieldz(i, j, k), 2));
  if (strcmp(component, "LogHz2Hx2") == 0)
    return log10(pow(hfieldz(i, j, k), 2) + pow(hfieldx(i, j, k), 2));
}

float grid_value_periodic_x(char *component, int i, int j, int k, int h) {
  float E_x, E_y, E_z, J_x, J_y, J_z, H_x, H_y, H_z;

  E_x = efieldx(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        iefieldx(i, j, k) * sin(2 * pi * h * wave_vector_x);
  E_y = efieldy(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        iefieldy(i, j, k) * sin(2 * pi * h * wave_vector_x);
  E_z = efieldz(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        iefieldz(i, j, k) * sin(2 * pi * h * wave_vector_x);
  J_x = jfieldx(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        ijfieldx(i, j, k) * sin(2 * pi * h * wave_vector_x);
  J_y = jfieldy(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        ijfieldy(i, j, k) * sin(2 * pi * h * wave_vector_x);
  J_z = jfieldz(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        ijfieldz(i, j, k) * sin(2 * pi * h * wave_vector_x);
  H_x = hfieldx(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        ihfieldx(i, j, k) * sin(2 * pi * h * wave_vector_x);
  H_y = hfieldy(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        ihfieldy(i, j, k) * sin(2 * pi * h * wave_vector_x);
  H_z = hfieldz(i, j, k) * cos(2 * pi * h * wave_vector_x) -
        ihfieldz(i, j, k) * sin(2 * pi * h * wave_vector_x);
  ...
  if (strcmp(component, "E^2") == 0)
    return pow(E_x, 2) + pow(E_y, 2) + pow(E_z, 2);
  if (strcmp(component, "H^2") == 0)
    return pow(H_x, 2) + pow(H_y, 2) + pow(H_z, 2);
  if (strcmp(component, "Sx") == 0)
    return E_y * H_z - E_z * H_y;
  if (strcmp(component, "Sy") == 0)
    return E_z * H_x - E_x * H_z;
  if (strcmp(component, "Sz") == 0)
    return E_x * H_y - E_y * H_x;
  if (strcmp(component, "LogE^2") == 0)
    return log10(pow(E_x, 2) + pow(E_y, 2) + pow(E_z, 2));
  if (strcmp(component, "LogH^2") == 0)
    return log10(pow(H_x, 2) + pow(H_y, 2) + pow(H_z, 2));

  if (strcmp(component, "EM_Energy") == 0 && meps(i, j, k) == 0.0)
    return 0.5 * eo * eps(i, j, k) * (pow(E_x, 2) + pow(E_y, 2) + pow(E_z, 2)) +
           0.5 * uo * ups * (pow(H_x, 2) + pow(H_y, 2) + pow(H_z, 2));
  if (strcmp(component, "EM_Energy") == 0 && meps(i, j, k) != 0.0)
    return 0.5 * eo * eps_m(i, j, k) *
               (pow(E_x, 2) + pow(E_y, 2) + pow(E_z, 2)) +
           0.5 * uo * ups * (pow(H_x, 2) + pow(H_y, 2) + pow(H_z, 2));
  if (strcmp(component, "EM_Energy2") == 0 && meps(i, j, k) == 0.0)
    return 0.0;
  if (strcmp(component, "EM_Energy2") == 0 && meps(i, j, k) != 0.0)
  ...
  if (strcmp(component, "Ex2Ey2") == 0)
    return pow(E_x, 2) + pow(E_y, 2);
  if (strcmp(component, "Ey2Ez2") == 0)
    return pow(E_y, 2) + pow(E_z, 2);
  if (strcmp(component, "Ez2Ex2") == 0)
    return pow(E_z, 2) + pow(E_x, 2);
  if (strcmp(component, "LogEx2Ey2") == 0)
    return log10(pow(E_x, 2) + pow(E_y, 2));
  if (strcmp(component, "LogEy2Ez2") == 0)
    return log10(pow(E_y, 2) + pow(E_z, 2));
  if (strcmp(component, "LogEz2Ex2") == 0)
    return log10(pow(E_z, 2) + pow(E_x, 2));
  if (strcmp(component, "Hx2Hy2") == 0)
    return pow(H_x, 2) + pow(H_y, 2);
  if (strcmp(component, "Hy2Hz2") == 0)
    return pow(H_y, 2) + pow(H_z, 2);
  if (strcmp(component, "Hz2Hx2") == 0)
    return pow(H_z, 2) + pow(H_x, 2);
  if (strcmp(component, "LogHx2Hy2") == 0)
    return log10(pow(H_x, 2) + pow(H_y, 2));
  if (strcmp(component, "LogHy2Hz2") == 0)
    return log10(pow(H_y, 2) + pow(H_z, 2));
  if (strcmp(component, "LogHz2Hx2") == 0)
    return log10(pow(H_z, 2) + pow(H_x, 2));
}

float grid_value_periodic_y(char *component, int i, int j, int k, int h) {
  float E_x, E_y, E_z, J_x, J_y, J_z, H_x, H_y, H_z;

  E_x = efieldx(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        iefieldx(i, j, k) * sin(2 * pi * h * wave_vector_y);
  E_y = efieldy(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        iefieldy(i, j, k) * sin(2 * pi * h * wave_vector_y);
  E_z = efieldz(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        iefieldz(i, j, k) * sin(2 * pi * h * wave_vector_y);
  J_x = jfieldx(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        ijfieldx(i, j, k) * sin(2 * pi * h * wave_vector_y);
  J_y = jfieldy(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        ijfieldy(i, j, k) * sin(2 * pi * h * wave_vector_y);
  J_z = jfieldz(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        ijfieldz(i, j, k) * sin(2 * pi * h * wave_vector_y);
  H_x = hfieldx(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        ihfieldx(i, j, k) * sin(2 * pi * h * wave_vector_y);
  H_y = hfieldy(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        ihfieldy(i, j, k) * sin(2 * pi * h * wave_vector_y);
  H_z = hfieldz(i, j, k) * cos(2 * pi * h * wave_vector_y) -
        ihfieldz(i, j, k) * sin(2 * pi * h * wave_vector_y);
...
float grid_value_periodic_xy(char *component, int i, int j, int k, int h,
  ...
  E_x = efieldx(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        iefieldx(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  E_y = efieldy(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        iefieldy(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  E_z = efieldz(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        iefieldz(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  J_x = jfieldx(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        ijfieldx(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  J_y = jfieldy(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        ijfieldy(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  J_z = jfieldz(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        ijfieldz(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  H_x = hfieldx(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        ihfieldx(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  H_y = hfieldy(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        ihfieldy(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
  H_z = hfieldz(i, j, k) * cos(2 * pi * h * wave_vector_x + v * wave_vector_y) -
        ihfieldz(i, j, k) * sin(2 * pi * h * wave_vector_x + v * wave_vector_y);
...
float efieldx(int i, int j, int k) {
  ...
  if (pisize <= i && (xparity == 1 || xparity == -1)) {
    i = isize - i;
    p = p * xparity;
  }
  if (pjsize <= j && (yparity == 1 || yparity == -1)) {
    j = jsize - j;
    p = p * (-yparity);
  }
  if (pksize <= k && (zparity == 1 || zparity == -1)) {
    k = ksize - k;
    p = p * (-zparity);
  }
...
float hfieldx(int i, int j, int k) {
  ...
  return p *
         (Hx[i][j][k] + Hx[i][j - 1][k] + Hx[i][j][k - 1] +
          Hx[i][j - 1][k - 1]) /
         4;
}
...
float hfieldy(int i, int j, int k) {
  ...
  return p *
         (Hy[i][j][k] + Hy[i][j][k - 1] + Hy[i - 1][j][k] +
          Hy[i - 1][j][k - 1]) /
         4;
}
...
float hfieldz(int i, int j, int k) {
  ...
  return p *
         (Hz[i][j][k] + Hz[i - 1][j][k] + Hz[i][j - 1][k] +
          Hz[i - 1][j - 1][k]) /
         4;
}
...
float ihfieldx(int i, int j, int k) {
  ...
  return p *
         (iHx[i][j][k] + iHx[i][j - 1][k] + iHx[i][j][k - 1] +
          iHx[i][j - 1][k - 1]) /
         4;
}
...
float ihfieldy(int i, int j, int k) {
  ...
  return p *
         (iHy[i][j][k] + iHy[i][j][k - 1] + iHy[i - 1][j][k] +
          iHy[i - 1][j][k - 1]) /
         4;
}
...
float ihfieldz(int i, int j, int k) {
  ...
  return p *
         (iHz[i][j][k] + iHz[i - 1][j][k] + iHz[i][j - 1][k] +
          iHz[i - 1][j - 1][k]) /
         4;
}
...
float eps(int i, int j, int k) {
  ...
  return (epsilonx[i][j][k] + epsilonx[i - 1][j][k] + epsilony[i][j][k] +
          epsilony[i][j - 1][k] + epsilonz[i][j][k] + epsilonz[i][j][k - 1]) /
         6;
}
...
float eps_m(int i, int j, int k) {
  ...
  // return(2*mepsilon[i][j][k] - wp*wp/(w*w+go*go) +
  // wp*wp*(w*w-go*go)/((w*w+go*go)*(w*w+go*go)) );  		//additional a
  // factor of 1/2 correction (ver.8.801)
  return (mepsilon[i][j][k] - 0.5 * wp * wp / (w * w + go * go) +
          0.5 * wp * wp * (w * w - go * go) /
              ((w * w + go * go) * (w * w + go * go)));
}

float eps_m2(int i, int j,
             int k) // for calculting kinetic energy of electrons (ver.8.78)
{
  float wp, w, go;
  ...
  return (wp * wp * (w * w - go * go) /
          ((w * w + go * go) * (w * w + go * go)));
}

float eps_m3(int i, int j,
             int k) // for calculting potential energy of electrons (ver.8.78)
{
  float wp, w, go;
...
void print_amp_and_phase(int mm) {
  ...
  for (j = jsize - 2; j >= 1; j--) {
    for (i = 1; i <= isize - 2; i++) {
      if ((i >= pmlil && i <= isize - pmlir) &&
          (j >= pmljl && j <= jsize - pmljr)) {
        fprintf(amp, "%f\t",
                sqrt(pow(Ex_cos[i][j][mm], 2) + pow(Ex_sin[i][j][mm], 2)));
  ...
  for (j = jsize - 2; j >= 1; j--) {
    for (i = 1; i <= isize - 2; i++) {
      if ((i >= pmlil && i <= isize - pmlir) &&
          (j >= pmljl && j <= jsize - pmljr)) {
        fprintf(amp, "%f\t",
                sqrt(pow(Ey_cos[i][j][mm], 2) + pow(Ey_sin[i][j][mm], 2)));
  ...
  for (j = jsize - 2; j >= 1; j--) {
    for (i = 1; i <= isize - 2; i++) {
      if ((i >= pmlil && i <= isize - pmlir) &&
          (j >= pmljl && j <= jsize - pmljr)) {
        fprintf(amp, "%f\t",
                sqrt(pow(Hx_cos[i][j][mm], 2) + pow(Hx_sin[i][j][mm], 2)));
  ...
  for (j = jsize - 2; j >= 1; j--) {
    for (i = 1; i <= isize - 2; i++) {
      if ((i >= pmlil && i <= isize - pmlir) &&
          (j >= pmljl && j <= jsize - pmljr)) {
        fprintf(amp, "%f\t",
                sqrt(pow(Hy_cos[i][j][mm], 2) + pow(Hy_sin[i][j][mm], 2)));
...
void print_real_and_imag_2n_size(int NROW, int mm) {
  ...
  deltar = (NROW - row) / 2 +
           (pmljl + 10 + pmljr + 10) /
               2; // triming out pml region that may cause numerical errors
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
          (j >= deltar && j < (NROW - deltar)))
        fprintf(
            stream, "%f\t",
            Ex_cos[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Ex_sin[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Ey_cos[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Ey_sin[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Hx_cos[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Hx_sin[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i >= deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Hy_cos[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
  ...
  for (j = NROW - 1; j >= 0; j--) {
    for (i = 0; i < NROW; i++) {
      if ((i > deltac && i < (NROW - deltac)) &&
        ...
        fprintf(
            stream, "%f\t",
            Hy_sin[i - deltac + (pmlil + 10)][j - deltar + (pmlir + 10)][mm]);
      else
        fprintf(stream, "%f\t", 0.0);
    }
    fprintf(stream, "\n");
...
void make_2n_size(int NROW, int mm) {
  ...
  char name_tEx_real[20], name_tEx_imag[20], name_tEy_real[20],
      name_tEy_imag[20];
  char name_tHx_real[20], name_tHx_imag[20], name_tHy_real[20],
      name_tHy_imag[20];
