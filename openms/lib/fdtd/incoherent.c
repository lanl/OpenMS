...
void incoherent_point_dipole(char *function, float x, float y, float z,
  ...
  if ((store_t == -100) || (store_t == t)) {
    ...
    if (one_round_turn == 0) {
      ...
      p_theta_aux =
          (float *)realloc(p_theta_aux, sizeof(float) * inc_dipole_num);
  ...
  {
    Jx = Gauss_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
         sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                      p_theta_aux[inc_dipole_num - 1]) *
         cos(p_phi[inc_dipole_num - 1]);
    Jy = Gauss_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
         sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                      p_theta_aux[inc_dipole_num - 1]) *
         sin(p_phi[inc_dipole_num - 1]);
    Jz = Gauss_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
         p_theta_aux[inc_dipole_num - 1];
    if ((use_periodic_x == 1 || use_periodic_y == 1) &&
        (wave_vector_x != 0.0 || wave_vector_y != 0.0)) {
      iJx = iGauss_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
            sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                         p_theta_aux[inc_dipole_num - 1]) *
            cos(p_phi[inc_dipole_num - 1]);
      iJy = iGauss_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
            sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                         p_theta_aux[inc_dipole_num - 1]) *
            sin(p_phi[inc_dipole_num - 1]);
      iJz = iGauss_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
            p_theta_aux[inc_dipole_num - 1];
    }
  }
  if (strcmp(function, "Lorentz") == 0) // Lorentzian envelope
  {
    Jx = Lorentz_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
         sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                      p_theta_aux[inc_dipole_num - 1]) *
         cos(p_phi[inc_dipole_num - 1]);
    Jy = Lorentz_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
         sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                      p_theta_aux[inc_dipole_num - 1]) *
         sin(p_phi[inc_dipole_num - 1]);
    Jz = Lorentz_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
         p_theta_aux[inc_dipole_num - 1];
    if ((use_periodic_x == 1 || use_periodic_y == 1) &&
        (wave_vector_x != 0.0 || wave_vector_y != 0.0)) {
      iJx = iLorentz_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
            sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                         p_theta_aux[inc_dipole_num - 1]) *
            cos(p_phi[inc_dipole_num - 1]);
      iJy = iLorentz_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
            sqrt(1 - p_theta_aux[inc_dipole_num - 1] *
                         p_theta_aux[inc_dipole_num - 1]) *
            sin(p_phi[inc_dipole_num - 1]);
      iJz = iLorentz_amp(frequency, p_eps[inc_dipole_num - 1], to, tdecay) *
            p_theta_aux[inc_dipole_num - 1];
    }
  }
...
float Gauss_amp(float frequency, float phase, long to, long tdecay) {
  if (to - 3 * tdecay < t && t < to + 3 * tdecay)
    return (sin(2 * pi * frequency * t / S_factor / ds_x / lattice_x + phase) *
            exp(-1.0 * pow((float)(t - to) / tdecay, 2.0)));
  else
    return (0.0);
}
...
float Lorentz_amp(float frequency, float phase, long to, long tdecay) {
  if (tdecay == 0)
    return (
        sin(2 * pi * frequency * (float)(t - to) / S_factor / ds_x / lattice_x +
            phase));
  else if (to <= t && t < to + 8 * tdecay)
    return (
        sin(2 * pi * frequency * (float)(t - to) / S_factor / ds_x / lattice_x +
            phase) *
        exp(-(float)(t - to) / tdecay));
  else
    return (0.0);
}
...
float iGauss_amp(float frequency, float phase, long to, long tdecay) {
  if (to - 3 * tdecay < t && t < to + 3 * tdecay)
    return (-cos(2 * pi * frequency * t / S_factor / ds_x / lattice_x + phase) *
            exp(-1.0 * pow((float)(t - to) / tdecay, 2.0)));
  else
    return (0.0);
}
...
float iLorentz_amp(float frequency, float phase, long to, long tdecay) {
  if (to <= t && t < to + 8 * tdecay) {
    if (tdecay == 0)
      return (-cos(2 * pi * frequency * (float)(t - to) / S_factor / ds_x /
                       lattice_x +
                   phase));
    else
      return (-cos(2 * pi * frequency * (float)(t - to) / S_factor / ds_x /
