...
void Gaussian_dipole_source_beam(char *component, int i,int j,float z,float frequency,float amp, float phase,long to,long tdecay);
void Lorentzian_dipole_source_beam(char *component, int i,int j,float z,float frequency,float amp, float phase,long to,long tdecay);
// since ver. 8.75, xxx_dipole_source_plane() were upgraded to xxx_dipole_source_beam()
...
void Gaussian_planewave(char *Ecomp, char *Hcomp, float position, float frequency, long to,long tdecay)
{
	...
	for(i=0; i<=p_mygrid_nx; i++)
		for(j=0; j<=p_mygrid_ny; j++)
		{
			phase = 2*pi*(wave_vector_x*i/(lattice_x) + wave_vector_y*j/(lattice_y)); 
			Gaussian_dipole_source_beam(Ecomp, i, j, position, frequency, 1.0, phase, to, tdecay);
			Gaussian_dipole_source_beam(Hcomp, i, j, position, frequency, 1.0, phase, to, tdecay);
		}
}
...
//////////////////////////////////////////////////////////////////////////// */
{
	...
	for(i=0; i<=p_mygrid_nx; i++)
		for(j=0; j<=p_mygrid_ny; j++)
		{
			r2 = (i-ic)*(i-ic)/((float)(lattice_x*lattice_x))+(j-jc)*(j-jc)/((float)(lattice_y*lattice_y));
			amp = (wo/w_z)*exp(-r2/(w_z*w_z) );
			phase = -(2*pi*(z_c-z)*frequency*n - eta_z + r2*2*pi*frequency*n/(2*R_z));
			Gaussian_dipole_source_beam(Ecomp, i, j, z, frequency, amp, phase, to, tdecay);
			Gaussian_dipole_source_beam(Hcomp, i, j, z, frequency, amp, phase, to, tdecay);
		}
}
...
void Gaussian_line_source(char *component, float position_x, float position_z, float frequency, float phase, long to,long tdecay)
{
	...
	for(j=0; j<=p_mygrid_ny; j++)
		Gaussian_dipole_source_line(component, position_x, j, position_z, frequency, phase, to, tdecay);
}
...
void Lorentzian_planewave(char *Ecomp, char *Hcomp, float position, float frequency, long to,long tdecay)
{
	...
	for(i=0; i<=p_mygrid_nx; i++)
		...
		{
			phase = 2*pi*(wave_vector_x*i/(lattice_x) + wave_vector_y*j/(lattice_y)); 
			Lorentzian_dipole_source_beam(Ecomp, i, j, position, frequency, 1.0, phase, to, tdecay);
			Lorentzian_dipole_source_beam(Hcomp, i, j, position, frequency, 1.0, phase, to, tdecay);
		}
}
...
//////////////////////////////////////////////////////////////////////////// */
{
	...
	for(i=0; i<=p_mygrid_nx; i++)
		for(j=0; j<=p_mygrid_ny; j++)
		{
			...
			amp = (wo/w_z)*exp(-r2/(w_z*w_z) );
			phase = -(2*pi*(z_c-z)*frequency*n - eta_z + r2*2*pi*frequency*n/(2*R_z));
			Lorentzian_dipole_source_beam(Ecomp, i, j, z, frequency, amp, phase, to, tdecay);
			Lorentzian_dipole_source_beam(Hcomp, i, j, z, frequency, amp, phase, to, tdecay);
		}
}
	...
	for(i=0; i<=p_mygrid_nx; i++)
		for(j=0; j<=p_mygrid_ny; j++)
		{
			if(strcmp(Ecomp,"Ex")==0)
			{
				...
				if(i>0) Ex[i-1][j][k]=Ex[i-1][j][k]+Ext/2;
			}
			if(strcmp(Ecomp,"Hy")==0)
			{
				Hy[i][j][k]=Hy[i][j][k]+Ext/2*sqrt(eo/uo)/4;
				if(i>0) Hy[i-1][j][k]=Hy[i-1][j][k]+Ext/2*sqrt(eo/uo)/4;
			}
		}
}
...
void Gaussian_dipole_source(char *component,float x,float y,float z,float frequency,float phase,long to,long tdecay)
{
	...
	//printf("test-zy: grid (%f %f %f) i = %d j = %d\n", x, y, z, i, j);
	if (i == -1 || j == -1) return;
	//printf("test-zy: grid (%f %f %f) is found in rank %d\n", x, y, z, mpi_rank);
...
void Gaussian_dipole_source_beam(char *component, int i,int j,float z,float frequency,float amp, float phase,long to,long tdecay)
{
	...
	if(strcmp(component,"Ex")==0)
	{
		Ex[i][j][k]=Ex[i][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)/2;
		if(i>=1)
			Ex[i-1][j][k]=Ex[i-1][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEx[i][j][k]=iEx[i][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)/2;
			if(i>=1)
				iEx[i-1][j][k]=iEx[i-1][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)/2;
		}
	}
	if(strcmp(component,"-Ex")==0)
	{
		Ex[i][j][k]=Ex[i][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)/2;
		if(i>=1)
			Ex[i-1][j][k]=Ex[i-1][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEx[i][j][k]=iEx[i][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)/2;
			if(i>=1)
				iEx[i-1][j][k]=iEx[i-1][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)/2;
		}
	}
	if(strcmp(component,"Ey")==0)
	{
		Ey[i][j][k]=Ey[i][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)/2;
		if(j>=1)
			Ey[i][j-1][k]=Ey[i][j-1][k]+amp*Gauss_amp(frequency,phase,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEy[i][j][k]=iEy[i][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)/2;
			if(j>=1)
				iEy[i][j-1][k]=iEy[i][j-1][k]+amp*iGauss_amp(frequency,phase,to,tdecay)/2;
		}
	}
	if(strcmp(component,"-Ey")==0)
	{
		Ey[i][j][k]=Ey[i][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)/2;
		if(j>=1)
			Ey[i][j-1][k]=Ey[i][j-1][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEy[i][j][k]=iEy[i][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)/2;
			if(j>=1)
				iEy[i][j-1][k]=iEy[i][j-1][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)/2;
		}
	}
	if(strcmp(component,"Ez")==0)
	{
		Ez[i][j][k]=Ez[i][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)/2;
		if(k>=1)
			Ez[i][j][k-1]=Ez[i][j][k-1]+amp*Gauss_amp(frequency,phase,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEz[i][j][k]=iEz[i][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)/2;
			if(k>=1)
				iEz[i][j][k-1]=iEz[i][j][k-1]+amp*iGauss_amp(frequency,phase,to,tdecay)/2;
		}
	}
	if(strcmp(component,"-Ez")==0)
	{
		Ez[i][j][k]=Ez[i][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)/2;
		if(k>=1)
			Ez[i][j][k-1]=Ez[i][j][k-1]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEz[i][j][k]=iEz[i][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)/2;
			if(k>=1)
				iEz[i][j][k-1]=iEz[i][j][k-1]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)/2;
		}
	}
	if(strcmp(component,"Hx")==0)
	{
		Hx[i][j][k]=Hx[i][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hx[i][j-1][k]=Hx[i][j-1][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hx[i][j][k-1]=Hx[i][j][k-1]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1 && k>=1)
			Hx[i][j-1][k-1]=Hx[i][j-1][k-1]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHx[i][j][k]=iHx[i][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHx[i][j-1][k]=iHx[i][j-1][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHx[i][j][k-1]=iHx[i][j][k-1]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1 && k>=1)
				iHx[i][j-1][k-1]=iHx[i][j-1][k-1]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"-Hx")==0)
	{
		Hx[i][j][k]=Hx[i][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hx[i][j-1][k]=Hx[i][j-1][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hx[i][j][k-1]=Hx[i][j][k-1]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1 && k>=1)
			Hx[i][j-1][k-1]=Hx[i][j-1][k-1]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHx[i][j][k]=iHx[i][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHx[i][j-1][k]=iHx[i][j-1][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHx[i][j][k-1]=iHx[i][j][k-1]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1 && k>=1)
				iHx[i][j-1][k-1]=iHx[i][j-1][k-1]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"Hy")==0)
	{
		Hy[i][j][k]=Hy[i][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hy[i][j][k-1]=Hy[i][j][k-1]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hy[i-1][j][k]=Hy[i-1][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && k>=1)
			Hy[i-1][j][k-1]=Hy[i-1][j][k-1]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHy[i][j][k]=iHy[i][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)*sqrt(eo/uo)/4;
			if(k>=1)
				iHy[i][j][k-1]=iHy[i][j][k-1]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHy[i-1][j][k]=iHy[i-1][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && k>=1)
				iHy[i-1][j][k-1]=iHy[i-1][j][k-1]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"-Hy")==0)
	{
		Hy[i][j][k]=Hy[i][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hy[i][j][k-1]=Hy[i][j][k-1]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hy[i-1][j][k]=Hy[i-1][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && k>=1)
			Hy[i-1][j][k-1]=Hy[i-1][j][k-1]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHy[i][j][k]=iHy[i][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHy[i][j][k-1]=iHy[i][j][k-1]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHy[i-1][j][k]=iHy[i-1][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && k>=1)
				iHy[i-1][j][k-1]=iHy[i-1][j][k-1]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"Hz")==0)
	{
		Hz[i][j][k]=Hz[i][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hz[i-1][j][k]=Hz[i-1][j][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hz[i][j-1][k]=Hz[i][j-1][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && j>=1)
			Hz[i-1][j-1][k]=Hz[i-1][j-1][k]+amp*Gauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHz[i][j][k]=iHz[i][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHz[i-1][j][k]=iHz[i-1][j][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHz[i][j-1][k]=iHz[i][j-1][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && j>=1)
				iHz[i-1][j-1][k]=iHz[i-1][j-1][k]+amp*iGauss_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"-Hz")==0)
	{
		Hz[i][j][k]=Hz[i][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hz[i-1][j][k]=Hz[i-1][j][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hz[i][j-1][k]=Hz[i][j-1][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && j>=1)
			Hz[i-1][j-1][k]=Hz[i-1][j-1][k]+amp*Gauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHz[i][j][k]=iHz[i][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHz[i-1][j][k]=iHz[i-1][j][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHz[i][j-1][k]=iHz[i][j-1][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && j>=1)
				iHz[i-1][j-1][k]=iHz[i-1][j-1][k]+amp*iGauss_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
}
...
void Lorentzian_dipole_source_beam(char *component,int i,int j,float z,float frequency,float amp, float phase,long to,long tdecay)
{
	...
	if(strcmp(component,"Ex")==0)
	{
		Ex[i][j][k]=Ex[i][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)/2;
		if(i>=1)
			Ex[i-1][j][k]=Ex[i-1][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEx[i][j][k]=iEx[i][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)/2;
			if(i>=1)
				iEx[i-1][j][k]=iEx[i-1][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)/2;
		}
	}
	if(strcmp(component,"-Ex")==0)
	{
		Ex[i][j][k]=Ex[i][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)/2;
		if(i>=1)
			Ex[i-1][j][k]=Ex[i-1][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEx[i][j][k]=iEx[i][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)/2;
			if(i>=1)
				iEx[i-1][j][k]=iEx[i-1][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)/2;
		}
	}
	if(strcmp(component,"Ey")==0)
	{
		Ey[i][j][k]=Ey[i][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)/2;
		if(j>=1)
			Ey[i][j-1][k]=Ey[i][j-1][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEy[i][j][k]=iEy[i][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)/2;
			if(j>=1)
				iEy[i][j-1][k]=iEy[i][j-1][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)/2;
		}
	}
	if(strcmp(component,"-Ey")==0)
	{
		Ey[i][j][k]=Ey[i][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)/2;
		if(j>=1)
			Ey[i][j-1][k]=Ey[i][j-1][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEy[i][j][k]=iEy[i][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)/2;
			if(j>=1)
				iEy[i][j-1][k]=iEy[i][j-1][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)/2;
		}
	}
	if(strcmp(component,"Ez")==0)
	{
		Ez[i][j][k]=Ez[i][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)/2;
		if(k>=1)
			Ez[i][j][k-1]=Ez[i][j][k-1]+amp*Lorentz_amp(frequency,phase,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEz[i][j][k]=iEz[i][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)/2;
			if(k>=1)
				iEz[i][j][k-1]=iEz[i][j][k-1]+amp*iLorentz_amp(frequency,phase,to,tdecay)/2;
		}
	}
	if(strcmp(component,"-Ez")==0)
	{
		Ez[i][j][k]=Ez[i][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)/2;
		if(k>=1)
			Ez[i][j][k-1]=Ez[i][j][k-1]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)/2;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iEz[i][j][k]=iEz[i][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)/2;
			if(k>=1)
				iEz[i][j][k-1]=iEz[i][j][k-1]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)/2;
		}
	}
	if(strcmp(component,"Hx")==0)
	{
		Hx[i][j][k]=Hx[i][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hx[i][j-1][k]=Hx[i][j-1][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hx[i][j][k-1]=Hx[i][j][k-1]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1 && k>=1)
			Hx[i][j-1][k-1]=Hx[i][j-1][k-1]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHx[i][j][k]=iHx[i][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHx[i][j-1][k]=iHx[i][j-1][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHx[i][j][k-1]=iHx[i][j][k-1]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1 && k>=1)
				iHx[i][j-1][k-1]=iHx[i][j-1][k-1]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"-Hx")==0)
	{
		Hx[i][j][k]=Hx[i][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hx[i][j-1][k]=Hx[i][j-1][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hx[i][j][k-1]=Hx[i][j][k-1]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1 && k>=1)
			Hx[i][j-1][k-1]=Hx[i][j-1][k-1]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHx[i][j][k]=iHx[i][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHx[i][j-1][k]=iHx[i][j-1][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHx[i][j][k-1]=iHx[i][j][k-1]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1 && k>=1)
				iHx[i][j-1][k-1]=iHx[i][j-1][k-1]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"Hy")==0)
	{
		Hy[i][j][k]=Hy[i][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hy[i][j][k-1]=Hy[i][j][k-1]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hy[i-1][j][k]=Hy[i-1][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && k>=1)
			Hy[i-1][j][k-1]=Hy[i-1][j][k-1]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHy[i][j][k]=iHy[i][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHy[i][j][k-1]=iHy[i][j][k-1]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHy[i-1][j][k]=iHy[i-1][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && k>=1)
			iHy[i-1][j][k-1]=iHy[i-1][j][k-1]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"-Hy")==0)
	{
		Hy[i][j][k]=Hy[i][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(k>=1)
			Hy[i][j][k-1]=Hy[i][j][k-1]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hy[i-1][j][k]=Hy[i-1][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && k>=1)
			Hy[i-1][j][k-1]=Hy[i-1][j][k-1]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHy[i][j][k]=iHy[i][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(k>=1)
				iHy[i][j][k-1]=iHy[i][j][k-1]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHy[i-1][j][k]=iHy[i-1][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && k>=1)
				iHy[i-1][j][k-1]=iHy[i-1][j][k-1]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"Hz")==0)
	{
		Hz[i][j][k]=Hz[i][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hz[i-1][j][k]=Hz[i-1][j][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hz[i][j-1][k]=Hz[i][j-1][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && j>=1)
		Hz[i-1][j-1][k]=Hz[i-1][j-1][k]+amp*Lorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHz[i][j][k]=iHz[i][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHz[i-1][j][k]=iHz[i-1][j][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHz[i][j-1][k]=iHz[i][j-1][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && j>=1)
				iHz[i-1][j-1][k]=iHz[i-1][j-1][k]+amp*iLorentz_amp(frequency,phase,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
	if(strcmp(component,"-Hz")==0)
	{
		Hz[i][j][k]=Hz[i][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1)
			Hz[i-1][j][k]=Hz[i-1][j][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(j>=1)
			Hz[i][j-1][k]=Hz[i][j-1][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if(i>=1 && j>=1)
			Hz[i-1][j-1][k]=Hz[i-1][j-1][k]+amp*Lorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
		{
			iHz[i][j][k]=iHz[i][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1)
				iHz[i-1][j][k]=iHz[i-1][j][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(j>=1)
				iHz[i][j-1][k]=iHz[i][j-1][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
			if(i>=1 && j>=1)
				iHz[i-1][j-1][k]=iHz[i-1][j-1][k]+amp*iLorentz_amp(frequency,phase+pi,to,tdecay)*sqrt(eo/uo)/4;
		}
	}
}
