...
class BaseMD(object):
    ...
    def initialize(
        ...
        if self.md_type == "Eh" and self.mol.lnacme:
            error_message = (
                "Ehrenfest dynamics needs evaluation of NACVs, check your QM object!"
            )
        ...
        if restart == "append":
            ...
            for md_idir in md_dir:
                if not os.path.exists(md_idir):
                    error_message = (
                        f"Directory {md_idir} to be appended for restart not found!"
                    )
    ...
    def write_final_xyz(self, md_dir, cstep):
        ...
        for iat in range(self.mol.natm):
            tmp += (
                ...
                + "".join(
                    [
                        f"{self.mol.veloc[iat, isp]:15.8f}"
                        for isp in range(self.mol.ndim)
                    ]
                )
            )
...
class BOMD(BaseMD):
    ...
    def kernel(self, qm, **kwargs):
        ...
        # Initialize
        base_dir, md_dir, qm_log_dir = self.initialize(
            qm, output_dir, save_qm_log, save_scr, restart
        )
