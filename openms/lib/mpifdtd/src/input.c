...
void make_epsilon()
{
    ...
    myprintf("-----------------------\n");
    myprintf("making the epsilon...\n");

    for(i = 0; i < m_mygrid_nx; i++)
    {
        printf("%d% \n", (int)100*i/m_mygrid_nx);

        for(j = 0; j < m_mygrid_ny; j++)
            for(k = 0; k < m_mygrid_nz; k++)
            {
                //printf("coordinate_x_y_z %d %d  %d;  %f %f  %f \n",i, j, k, mygrid_x[i], mygrid_y[j], mygrid_z[k]);
                epsilonx[i][j][k]=average_epsilon(mygrid_x[i]+0.5, mygrid_x[j], mygrid_z[k]);
                epsilony[i][j][k]=average_epsilon(mygrid_x[i], mygrid_y[j]+0.5, mygrid_z[k]);
                epsilonz[i][j][k]=average_epsilon(mygrid_x[i], mygrid_y[j], mygrid_z[k]+0.5);
            }
    }

    free(object);

    myprintf("make_epsilon...ok\n");
}
...
int in_object(int n,float i,float j,float k)
{
	...
	int temp;
        ...
        // FIXME: the coordinates when MPION need corrections
