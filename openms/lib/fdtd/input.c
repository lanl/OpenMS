...
void generate_random_2D(float x_min, float x_max, float y_min, float y_max,
                        float radius, float *xr, float *yr, int i);
void generate_random_3D(float x_min, float x_max, float y_min, float y_max,
                        float z_min, float z_max, float radius, float *xr,
                        float *yr, float *zr, int i);
float rand_distance_2D(float x_scan, float y_scan, float x_temp, float y_temp);
float rand_distance_3D(float x_scan, float y_scan, float z_scan, float x_temp,
                       float y_temp, float z_temp);
...
void background(float epsilon) {
  ...
  for (i = 0; i < misize; i++)
    for (j = 0; j < mjsize; j++)
      for (k = 0; k < mksize; k++)
        epsilonx[i][j][k] = epsilony[i][j][k] = epsilonz[i][j][k] =
            back_epsilon;

  printf("background...ok\n");
}

void input_object(char *shape, char *matrix_file, float centerx, float centery,
                  float centerz, float size1, float size2, float size3,
                  float epsilon) {
  objectn++;
  ...
  if (strcmp(shape, "contour") == 0) {
    ...
    object[objectn - 1].centerk =
        (centerz); // for non_uniform_grid(), the meaning changed
    object[objectn - 1].size1 = size1; // discrimination level
    object[objectn - 1].size2 =
        (size2); // height  // for non_uniform_grid() .....
    ...
    object[objectn - 1].size2 =
        (size2); // height  // for non_uniform_grid() ....
  } else if (strcmp(shape, "rodX") ==
             0) //// not applicable for non_uniform_grid()
  {
    object[objectn - 1].centeri = (centerx + xcenter) * lattice_x;
    object[objectn - 1].centerj = (centery + ycenter) * lattice_y;
             ...
             0) //// not applicable for non_uniform_grid()
  {
    ...
    object[objectn - 1].size3 =
        (size3); // height  // for non_uniform_grid() ....
             ...
             0) //// not applicable for non_uniform_grid()
  {
    ...
    object[objectn - 1].size1 = size1 * lattice_x; // radius
    object[objectn - 1].size2 =
        (size2); // height (centerz); // for non_uniform_grid() ....
    object[objectn - 1].size3 = size3; // aspect ratio=ry/rx
  } else if (strcmp(shape, "ellipsoidal") ==
             0) //// not applicable for non_uniform_grid()
  {
    object[objectn - 1].centeri = (centerx + xcenter) * lattice_x;
    object[objectn - 1].centerj = (centery + ycenter) * lattice_y;
  ...
  } else                                 // "block"
  {
    ...
    object[objectn - 1].size3 =
        (size3); // for non_uniform_grid(), the meaning changed
  }
}

void input_object_Euler_rotation(char *shape, char *matrix_file, float centerx,
                                 float centery, float centerz, float size1,
                                 float size2, float size3, float alpha,
...
// alpha, beta, gamma : 3- Euler angles
{
  ...
  //// in the below, we define various input parameters case by case. ////
  //// Contrary to "input_object()", 'Euler rotaion' cannot be applied to
  ///"sphere" and "contour"////
  ...
  if (strcmp(shape, "rod") == 0) {
    ...
    object[objectn - 1].size2 = size2 * lattice_y; // ry
    object[objectn - 1].size3 = size3 * lattice_z; // rz
    strcpy(object[objectn - 1].shape,
           "Rellipsoidal"); // Euler Rotation indicator
...
void random_object(char *shape, float radius, float height, float epsilon,
                   float x_min, float x_max, float y_min, float y_max,
                   float z_min, float z_max, int gen_number, int seed) {
  int i;
  ...
  if (strcmp(shape, "rod") == 0) {
    ...
    for (i = 0; i < gen_number; i++) {
      ...
      input_object("rod", EMP, xr[i], yr[i], 0 + shift, radius, height, 0,
                   epsilon);
    }
  }
  ...
  if (strcmp(shape, "sphere") == 0) {
    ...
    for (i = 0; i < gen_number; i++) {
      generate_random_3D(x_min, x_max, y_min, y_max, z_min, z_max, radius, xr,
                         yr, zr, i);
...
void generate_random_2D(float x_min, float x_max, float y_min, float y_max,
  ...
  xr_temp =
      ((float)rand() / ((float)(RAND_MAX) + (float)(1))) * (x_max - x_min) +
      x_min;
  yr_temp =
      ((float)rand() / ((float)(RAND_MAX) + (float)(1))) * (y_max - y_min) +
      y_min;

  if (i > 0) {
    while (1) {
      for (scan = 0; scan < i; scan++)
        if (rand_distance_2D(xr[scan], yr[scan], xr_temp, yr_temp) <
            2 * radius) {
          scan = -10;
          break;
        }
      ...
      if (scan == i) {
        ...
        xr_temp = ((float)rand() / ((float)(RAND_MAX) + (float)(1))) *
                      (x_max - x_min) +
                  x_min;
        yr_temp = ((float)rand() / ((float)(RAND_MAX) + (float)(1))) *
                      (y_max - y_min) +
                  y_min;
      }
    }
...
float rand_distance_2D(float x_scan, float y_scan, float x_temp, float y_temp) {
  return (sqrt((x_scan - x_temp) * (x_scan - x_temp) +
               (y_scan - y_temp) * (y_scan - y_temp)));
}
...
void generate_random_3D(float x_min, float x_max, float y_min, float y_max,
  ...
  yr_temp =
      ...
      y_min;
  zr_temp =
      ((float)rand() / ((float)(RAND_MAX) + (float)(1))) * (z_max - z_min) +
      z_min;

  if (i > 0) {
    while (1) {
      for (scan = 0; scan < i; scan++)
        if (rand_distance_3D(xr[scan], yr[scan], zr[scan], xr_temp, yr_temp,
                             zr_temp) < 2 * radius) {
          scan = -10;
          break;
        }
      ...
      if (scan == i) {
        ...
        yr_temp = ((float)rand() / ((float)(RAND_MAX) + (float)(1))) *
                      (y_max - y_min) +
                  y_min;
        zr_temp = ((float)rand() / ((float)(RAND_MAX) + (float)(1))) *
                      (z_max - z_min) +
                  z_min;
      }
    }
...
float average_epsilon(float i, float j, float k) {
  ...
  for (m = 0; m < 1000; m++)
    partial_epsilon[m] = back_epsilon;

  for (n = 0; n < objectn; n++) {

    if (in_object(n, i - 0.5, j - 0.5, k - 0.5) == 0 &&
        in_object(n, i - 0.5, j - 0.5, k + 0.5) == 0 &&
        in_object(n, i - 0.5, j + 0.5, k - 0.5) == 0 &&
        in_object(n, i - 0.5, j + 0.5, k + 0.5) == 0 &&
        in_object(n, i + 0.5, j - 0.5, k - 0.5) == 0 &&
        in_object(n, i + 0.5, j - 0.5, k + 0.5) == 0 &&
    ...
    else if (in_object(n, i - 0.5, j - 0.5, k - 0.5) == 1 &&
             in_object(n, i - 0.5, j - 0.5, k + 0.5) == 1 &&
             in_object(n, i - 0.5, j + 0.5, k - 0.5) == 1 &&
             in_object(n, i - 0.5, j + 0.5, k + 0.5) == 1 &&
             in_object(n, i + 0.5, j - 0.5, k - 0.5) == 1 &&
             in_object(n, i + 0.5, j - 0.5, k + 0.5) == 1 &&
      ...
      for (m = 0; m < 1000; m++)
        partial_epsilon[m] = object[n].epsilon;
      partial = 0;
    }
...
int in_object(int n, float i, float j, float k) {
  ...
  if (strcmp(object[n].shape, "Rrod") != 0 &&
      strcmp(object[n].shape, "Rellipse") != 0 &&
      strcmp(object[n].shape, "Rellipsoidal") != 0 &&
      strcmp(object[n].shape, "Rblock") != 0) {
    if (strcmp(object[n].shape, "rod") == 0) {
      if ((i - object[n].centeri) * (i - object[n].centeri) +
                  (j - object[n].centerj) * (j - object[n].centerj) <=
              object[n].size1 * object[n].size1 &&
          non_uniform_z_to_i(object[n].centerk - 0.5 * object[n].size2) <= k &&
          k <= non_uniform_z_to_i(object[n].centerk + 0.5 * object[n].size2))
        return 1;
      else
        return 0;
    }
      ...
      if ((j - object[n].centerj) * (j - object[n].centerj) +
                  (k - object[n].centerk) * (k - object[n].centerk) <=
              object[n].size1 * object[n].size1 &&
          object[n].centeri - object[n].size2 / 2 <= i &&
          i <= object[n].centeri + object[n].size2 / 2)
        return 1;
      else
        return 0;
    }
      ...
      if ((k - object[n].centerk) * (k - object[n].centerk) +
                  (i - object[n].centeri) * (i - object[n].centeri) <=
              object[n].size1 * object[n].size1 &&
          object[n].centerj - object[n].size2 / 2 <= j &&
          j <= object[n].centerj + object[n].size2 / 2)
        return 1;
      else
        return 0;
    }
      ...
      if ((i - object[n].centeri) * (i - object[n].centeri) +
          ...
          (i - object[n].centeri) * (i - object[n].centeri) +
                  (j - object[n].centerj) * (j - object[n].centerj) <=
              object[n].size2 * object[n].size2 &&
          non_uniform_z_to_i(object[n].centerk - 0.5 * object[n].size3) <= k &&
          k <= non_uniform_z_to_i(object[n].centerk + 0.5 * object[n].size3))
        return 1;
      else
        return 0;
    }
      ...
      //  size3 : height
      ///////////////////////////////////////////////
      temp = (object[n].size1 - object[n].size2) *
                 (non_uniform_i_to_z(k) - object[n].centerk) / object[n].size3 +
             0.5 * (object[n].size1 +
                    object[n].size2); // radius varying as a function of 'k'

      if ((i - object[n].centeri) * (i - object[n].centeri) +
                  (j - object[n].centerj) * (j - object[n].centerj) <=
              temp * temp &&
          (k <=
           non_uniform_z_to_i(object[n].centerk + 0.5 * object[n].size3)) &&
          (k >= non_uniform_z_to_i(object[n].centerk - 0.5 * object[n].size3)))
        return 1;
      else
        return 0;
      ...
      if (object[n].centeri - object[n].size1 / 2 <= i &&
          i <= object[n].centeri + object[n].size1 / 2 &&
          object[n].centerj - object[n].size2 / 2 <= j &&
          j <= object[n].centerj + object[n].size2 / 2 &&
          non_uniform_z_to_i(object[n].centerk - 0.5 * object[n].size3) <= k &&
          k <= non_uniform_z_to_i(object[n].centerk + 0.5 * object[n].size3))
      ...
      if ((-object[n].centeri * CUTFACTOR <=
           (i - (xcenter * lattice_x)) / object[n].size3) &&
          ((i - (xcenter * lattice_x)) / object[n].size3 <=
           (object[n].col - object[n].centeri) * CUTFACTOR) &&
          (-object[n].centerj * CUTFACTOR <=
           (j - (ycenter * lattice_y)) / object[n].size3) &&
          ((j - (ycenter * lattice_y)) / object[n].size3 <=
           (object[n].row - object[n].centerj) * CUTFACTOR) &&
          (non_uniform_z_to_i(object[n].centerk - 0.5 * object[n].size2) <=
           k) &&
          (k <=
           non_uniform_z_to_i(object[n].centerk + 0.5 * object[n].size2))) {
        I = floor(object[n].centeri +
                  (i - (xcenter * lattice_x)) / object[n].size3);
        J = floor(object[n].centerj +
                  (j - (ycenter * lattice_y)) / object[n].size3);
  ...
  ///strcmp(object[n].shape,"Rblock")==0)
  {
    //// Transform (i,j,k) --> (X,Y,Z), the coordinate with respect to the
    ///object
    X = (cos_a[n] * cos_c[n] - sin_a[n] * cos_b[n] * sin_c[n]) *
            (i - object[n].centeri) +
        (sin_a[n] * cos_c[n] + cos_a[n] * cos_b[n] * sin_c[n]) *
            (j - object[n].centerj) +
        (sin_b[n] * sin_c[n]) * (k - object[n].centerk) + object[n].centeri;
    Y = -(cos_a[n] * sin_c[n] + sin_a[n] * cos_b[n] * cos_c[n]) *
            (i - object[n].centeri) -
        (sin_a[n] * sin_c[n] - cos_a[n] * cos_b[n] * cos_c[n]) *
            (j - object[n].centerj) +
        (sin_b[n] * cos_c[n]) * (k - object[n].centerk) + object[n].centerj;
    Z = (sin_a[n] * sin_b[n]) * (i - object[n].centeri) -
        (cos_a[n] * sin_b[n]) * (j - object[n].centerj) +
        cos_b[n] * (k - object[n].centerk) + object[n].centerk;

    if (strcmp(object[n].shape, "Rrod") == 0) {
      if ((X - object[n].centeri) * (X - object[n].centeri) +
                  (Y - object[n].centerj) * (Y - object[n].centerj) <=
              object[n].size1 * object[n].size1 &&
          object[n].centerk - object[n].size2 / 2 <= Z &&
          Z <= object[n].centerk + object[n].size2 / 2)
        return 1;
      else
        return 0;
    }
    ...
    if (strcmp(object[n].shape, "Rellipsoidal") == 0) {
      if ((X - object[n].centeri) * (X - object[n].centeri) /
              ...
              (Y - object[n].centerj) * (Y - object[n].centerj) /
                  ((object[n].size2) * (object[n].size2)) +
              (Z - object[n].centerk) * (Z - object[n].centerk) /
                  ((object[n].size3) * (object[n].size3)) <=
          1)
        return 1;
      else
      ...
      if (object[n].centeri - object[n].size1 / 2 <= X &&
          X <= object[n].centeri + object[n].size1 / 2 &&
          object[n].centerj - object[n].size2 / 2 <= Y &&
          Y <= object[n].centerj + object[n].size2 / 2 &&
          object[n].centerk - object[n].size3 / 2 <= Z &&
          Z <= object[n].centerk + object[n].size3 / 2)
        return 1;
      else
        return 0;
    }
