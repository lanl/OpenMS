...
float sigmax(float a) {
  ...
  if (use_periodic_x == 0) // no use of periodic boundary condition
  {
    ...
    if (a < pmlil + 0.5)
      return sigma_maxl * pow(-(a - (pmlil + 0.5)) / pmlil, orderxl);
    else if (a < isize - pmlir - 1)
      return 0.0;
    else
      return sigma_maxr * pow((a - (isize - pmlir - 1)) / pmlir, orderxr);
...
float sigmay(float a) {
  ...
  if (use_periodic_y == 0) // no use of periodic boundary condition
  {
    ...
    if (a < pmljl + 0.5)
      return sigma_maxl * pow(-(a - (pmljl + 0.5)) / pmljl, orderyl);
    else if (a < jsize - pmljr - 1)
      return 0.0;
    else
      return sigma_maxr * pow((a - (jsize - pmljr - 1)) / pmljr, orderyr);
  } else // use of periodic boundary condition
    return 0.0;
}
...
float sigmaz(float a) {
  ...
  if (a < pmlkl + 0.5)
    return sigma_maxl * pow(-(a - (pmlkl + 0.5)) / pmlkl, orderzl);
  else if (a < ksize - pmlkr - 1)
    return 0.0;
  else
    return sigma_maxr * pow((a - (ksize - pmlkr - 1)) / pmlkr, orderzr);
}
...
float cpmlbx(float a, float pmlbl, float pmlbr) {
  ...
  if (use_periodic_x == 0) // no use of periodic boundary condition
  {
    ...
    } else if (a < isize - pmlbr - 1)
      return 1.0;
    else {
      sigmae = sigmaCPML * pow((a - (isize - pmlbr - 1.0)) / pmlbr, m_pml);
      alphae = alphaCPML * pow((isize - 1.0 - a) / pmlbr, ma_pml);
      kappae = 1.0 + (kappaCPML - 1.0) *
                         pow((a - (isize - pmlbr - 1.0)) / pmlbr, m_pml);
      return bx = exp(-(sigmae / kappae + alphae) * dt_nm / eo);
    }
  } else
    return 1.0;
