...
void non_uniform_grid(char *component, float z_i, float z_f, int nlz) {
  ...
  ngrid =
      (struct ngrid_info *)realloc(ngrid, sizeof(struct ngrid_info) * ngrid_n);
...
void quick_sort_nz_start(struct ngrid_info *ngrid_s, int left, int right) {
  ...
  j = right;
  for (;;) {
    while ((ngrid_s[i].nz_start < pivot) && (i < right))
      i++;
    while (pivot < ngrid_s[j].nz_start)
      j--;
    if (i >= j)
      break;
    temp = ngrid_s[i];
  ...
  if (left < i - 1)
    quick_sort_nz_start(ngrid_s, left, i - 1);
  if (j + 1 < right)
    quick_sort_nz_start(ngrid_s, j + 1, right);
}
...
int non_uniform_z_to_i(float z) {
  ...
  -------end[2]  ----------- base_z[3] & base_grid[3]   <-- index runs to
  (ngrid_n) non_uniform[2]
  -------start[2]
  ...
  while (n < ngrid_n) {
    if (z <= ngrid[n].nz_start) {
      ...
      return (floor(0.5 + ((ngrid[n].nz_start - base_z[n]) * lattice_z +
                           (z - ngrid[n].nz_start) * ngrid[n].lattice_nz)) +
              base_grid[n]);
    } else
      n++;
  }
...
float non_uniform_i_to_z(int i) {
  ...
  // Note that this function cannot return the original 'z' value, due to the
  // rounding (loss of information).
  ...
  while (n < ngrid_n) {
    ...
    } else if ((i > non_uniform_z_to_i(ngrid[n].nz_start)) &&
               (i <= non_uniform_z_to_i(ngrid[n].nz_end))) {
      return (((float)(non_uniform_z_to_i(ngrid[n].nz_start) - base_grid[n])) /
                  ((float)lattice_z) +
              ((float)(i - non_uniform_z_to_i(ngrid[n].nz_start))) /
                  ((float)ngrid[n].lattice_nz) +
              base_z[n]);
    } else
      n++;
  }
...
void set_default_parameter(float S) {
  ...
  dt = 1 / light_speed /
       S_factor; // S=Couriant parameter for numerical stability
  dt_nm = dt * 1e-9;
  ...
  pksize = ksize - 1;
  cksize =
      ksize / 2; // in case of using non_uniform_grid(), this will not be used.
  ...
  ///////////////////////////////////////////////////
  printf("creating non uniform z map ok\n"); ////////////
  stream = fopen("dsz_map.dat", "wt");
  for (i = mksize - 1; i >= 0; i--)
    fprintf(stream, "%g\n", ds_nz[i]);
...
void set_sigma_order(float oxl, float oxr, float oyl, float oyr, float ozl,
  ...
  orderxl = oxl;
  orderyl = oyl;
  orderzl = ozl; // default value = 3.5
  orderxr = oxr;
  orderyr = oyr;
  orderzr = ozr; // default value = 3.5
}
...
void set_sigma_max(float axl, float axr, float ayl, float ayr, float azl,
  ...
  sig_axl = axl;
  sig_ayl = ayl;
  sig_azl = azl; // default value = 1.0
  sig_axr = axr;
  sig_ayr = ayr;
  sig_azr = azr; // default value = 1.0
}
...
int lcm3(int a, int b, int c) { return (lcm2(a, lcm2(b, c))); }
...
void generate_base_grid() {
  ...
  base_grid[0] = 0;
  printf("base_grid[0] = %d\n", base_grid[0]);

  for (i = 1; i <= ngrid_n; i++) {
    if (i == 1)
      base_grid[1] =
          (ngrid[0].nz_end - ngrid[0].nz_start) * ngrid[0].lattice_nz +
          (ngrid[0].nz_start + 0.5 * zsize) * lattice_z;
    else
      base_grid[i] = base_grid[i - 1] +
                     (ngrid[i - 1].nz_end - ngrid[i - 1].nz_start) *
                         ngrid[i - 1].lattice_nz +
                     (ngrid[i - 1].nz_start - ngrid[i - 2].nz_end) * lattice_z;
    printf("base_grid[%d] = %d\n", i, base_grid[i]);
  }
}
