...
int coord2index(float mygrid_x[], int size, float target){
    ...
    // Loop through the array to find the closest index
    for (int j = 0; j < size; j++) {
        float diff = fabs(mygrid_x[j] - target);

        // If this element is closer to target than the previous closest, update closestIndex and minDiff
        if (diff < minDiff) {
            minDiff = diff;
            closestIndex = j;
        }
    }
    printf("minDif = %f  index = %d\n", minDiff, closestIndex);
...
{
    // FIXME: output epsilon in mpi setting
    ...
    if(strcmp("x",plane)==0)
    {
	// FIXME, only specific ranks (that contains the x coordinates) need to output
	// i.e., we need to coporate a set of related nodes for the outputs
        float target = floor(0.5+((value+xcenter)*lattice_x));
	i = coord2index(mygrid_x, mygrid_nx, target);
	printf("test-zy, xcenter = %f target %f  i = %d\n", xcenter, target, i);
    ...
    if(strcmp("y",plane)==0)
    {
        float target = floor(0.5+((value+ycenter)*lattice_y));
	j = coord2index(mygrid_y, mygrid_ny, target);
        for(k=k_range;k>=1;k--)
        {
            for(mz=0; mz<nz_multiple(k); mz++) // for non_uniform_grid multiplication
            {
    ...
    if(strcmp("z",plane)==0)
    {
	// FIXME: non_uniform_z_to_i in mpi setting need fix
	// now simplest way is DONT use mpi for z direction
#ifdef MPION
        float target = non_uniform_z_to_i(value);
	k = coord2index(mygrid_z, mygrid_nz, target);
#else
...
float efieldx(int i,int j,int k)
{
    ...
    if(p_mygrid_nx<=i && (xparity==1 || xparity==-1)) {i=mygrid_nx-i;p=p*xparity;}
    if(p_mygrid_ny<=j && (yparity==1 || yparity==-1)) {j=mygrid_ny-j;p=p*(-yparity);}
    if(p_mygrid_nz<=k && (zparity==1 || zparity==-1)) {k=mygrid_nz-k;p=p*(-zparity);}
...
{
    if(p_mygrid_nx<=i && (xparity==1 || xparity==-1)) i=mygrid_nx-i;
    if(p_mygrid_ny<=j && (yparity==1 || yparity==-1)) j=mygrid_ny-j;
    if(p_mygrid_nz<=k && (zparity==1 || zparity==-1)) k=mygrid_nz-k;
...
void print_amp_and_phase(int mm)
{
    ...
    for(j=mygrid_ny-2; j>=1; j--)
    {
        for(i=1; i<=mygrid_nx-2; i++)
        {
            if( (i >= pmlil && i <=mygrid_nx-pmlir) && (j >= pmljl && j <= mygrid_ny-pmljr))
            {
...
void print_real_and_imag_2n_size(int NROW, int mm)
{
    ...
    /////////////////////////////////////////////////
    row = mygrid_ny-2; col = mygrid_nx-2; 
