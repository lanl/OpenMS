...
class HHModel(object):
    def __init__(
        ...
        if gij is not None:
            ...
            self.gmat[:, idx + 1 + L, idx + L] = self.gmat[
                :, idx + L, idx + 1 + L
            ] = gij
            if bc == "p":
    ...
    def fci(self, nphonon=20, nroots=1):
        ...
        M = self.M
        e, c = kernel(
            t, u, gmat, w, L, N, M, nphonon, space="r", verbose=4, nroots=nroots
        )
        return e, c
    ...
    def g_fock(self):
        ...
        Fov = numpy.einsum("pi,pq,qa->ia", Co, F, Cv) - 2 * numpy.einsum(
            "I,pi,Ipq,qa->ia", self.xi, Co, self.gmat, Cv
        )
        Fvo = numpy.einsum("pa,pq,qi->ai", Cv, F, Co) - 2 * numpy.einsum(
            "I,pa,Ipq,qi->ai", self.xi, Cv, self.gmat, Co
        )
