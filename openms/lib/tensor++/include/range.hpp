...
public:
  ...
  auto range() const noexcept { return hi_ - lo_; }
...
//!
//!      Index startA = inter_lo + ((lo_ - inter_lo) % step_ + step_) % step_;
//!      Index startB = inter_lo + ((rhs.lo() - inter_lo) % rhs.step() + rhs.step()) % rhs.step();
...
//! */
//!static inline Range range(Index lo, Index hi, Index step = 1) { return Range{lo, hi, step}; }
//!
//!/**
