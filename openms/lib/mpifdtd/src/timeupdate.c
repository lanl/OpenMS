...
void coefficient_cpml()
{
	...
	for(i=0;i<m_mygrid_nx;i++)
	{
		...
		if(i>mygrid_nx/2) ggx[i] = cpmlax(i-0.5,pmlil,pmlir);
		if(i>mygrid_nx/2) hhx[i] = cpmlbx(i-0.5,pmlil,pmlir);
		if(i>mygrid_nx/2) ffx[i] =kappa_x(i-0.5,pmlil,pmlir);
		printf("aax= %d %f %f %f %f %f %f \n", i, aax[i], bbx[i], ggx[i], hhx[i], ccx[i], ffx[i]);
	}
	...
	for(j=0;j<m_mygrid_ny;j++)
	{
		...
		if(j>mygrid_ny/2) ggy[j] = cpmlax(j-0.5,pmljl,pmljr);
		if(j>mygrid_ny/2) hhy[j] = cpmlbx(j-0.5,pmljl,pmljr);
		if(j>mygrid_ny/2) ffy[j] =kappa_x(j-0.5,pmljl,pmljr);
		printf("aay= %d %f %f %f %f %f %f \n", j, aay[j], bby[j], ggy[j], hhy[j], ccy[j], ffy[j]);
	}
	...
	for(k=0;k<m_mygrid_nz;k++)
	{
		...
		if(k>mygrid_nz/2) ggz[k] = cpmlax(k-0.5,pmlkl,pmlkr);
		if(k>mygrid_nz/2) hhz[k] = cpmlbx(k-0.5,pmlkl,pmlkr);
		if(k>mygrid_nz/2) ffz[k] =kappa_x(k-0.5,pmlkl,pmlkr);
		printf("aaz= %d %f %f %f %f %f %f \n", k, aaz[k], bbz[k], ggz[k], hhz[k], ccz[k], ffz[k]);
	}
...
void propagate()
{
	...
	//******* real E-field ******//
	for(i=1;i<p_mygrid_nx;i++)
	for(j=1;j<p_mygrid_ny;j++)
	for(k=1;k<p_mygrid_nz;k++)
	{
		if(position[i][j][k]==1 && mepsilon[i][j][k]==0.0) // non-metal
			normal_E_field_update(i, j, k);
		else if(position[i][j][k]==1 && mepsilon[i][j][k]==100) // molecular (include the feedbakc of molecular)
		...
		else if(position[i][j][k]==0 && (mepsilon[i][j][k]!=0.0 && mepsilon[i][j][k]!=1000)) // metal & PML
	...
	if((use_periodic_x == 1 || use_periodic_y == 1) && (wave_vector_x!=0.0 || wave_vector_y!=0.0))
	{
		...
		if(position[i][j][k]==1 && mepsilon[i][j][k]==0.0) // non-metal
			...
			metal_iE_field_update(i, j, k);  // PML update not used for this region
		else if(position[i][j][k]==1 && mepsilon[i][j][k]==1000) // PCM
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		if(position[i][j][k]==1 && (mepsilon[i][j][k]!=1000))
			...
			//cpml_H_field_update(i, j, k);  // only for non-periodic and none-symmetry
...
void molecular_E_field_update(int i, int j, int k)
{
	...
	Ex[i][j][k] = Ex[i][j][k] + (dt/eo/epsilonx[i][j][k])*((Hz[i][j][k]-Hz[i][j-1][k])/ds_y - (Hy[i][j][k]-Hy[i][j][k-1])*(2/(ds_nz[k-1]+ds_nz[k]))-Px);
	Ey[i][j][k] = Ey[i][j][k] + (dt/eo/epsilony[i][j][k])*((Hx[i][j][k]-Hx[i][j][k-1])*(2/(ds_nz[k-1]+ds_nz[k])) - (Hz[i][j][k]-Hz[i-1][j][k])/ds_x-Py);
	Ez[i][j][k] = Ez[i][j][k] + (dt/eo/epsilonz[i][j][k])*((Hy[i][j][k]-Hy[i-1][j][k])/ds_x - (Hx[i][j][k]-Hx[i][j-1][k])/ds_y-Pz);
}
...
void cpml_E_field_update(int i, int j, int k)
{
	...
	Ex[i][j][k] = Ex[i][j][k] + (dt/eo/epsilonx[i][j][k])*((Hz[i][j][k]-Hz[i][j-1][k])/ds_y/ccy[j] - (Hy[i][j][k]-Hy[i][j][k-1])*2.0/(ds_nz[k-1]+ds_nz[k])/ccz[k]);
	Ey[i][j][k] = Ey[i][j][k] + (dt/eo/epsilony[i][j][k])*((Hx[i][j][k]-Hx[i][j][k-1])*2.0/(ds_nz[k-1]+ds_nz[k])/ccz[k] - (Hz[i][j][k]-Hz[i-1][j][k])/ds_x/ccx[i]);
	Ez[i][j][k] = Ez[i][j][k] + (dt/eo/epsilonz[i][j][k])*((Hy[i][j][k]-Hy[i-1][j][k])/ds_x/ccx[i] - (Hx[i][j][k]-Hx[i][j-1][k])/ds_y/ccy[j]);

	//printf("testcpml_ %d %d %d %f %f %f \n", i, j, k, aax[i], aay[j], aaz[k]);

	psi_Exy[i][j][k] = bby[j]*psi_Exy[i][j][k] + aay[j]*(Hz[i][j][k]-Hz[i][j-1][k])/ds_y;
	psi_Exz[i][j][k] = bbz[k]*psi_Exz[i][j][k] + aaz[k]*(Hy[i][j][k-1]-Hy[i][j][k])*2.0/(ds_nz[k-1]+ds_nz[k]);
	Ex[i][j][k] = Ex[i][j][k] + dt/eo*(psi_Exy[i][j][k] + psi_Exz[i][j][k]);

	psi_Eyz[i][j][k] = bbz[k]*psi_Eyz[i][j][k] + aaz[k]*(Hx[i][j][k]-Hx[i][j][k-1])*2.0/(ds_nz[k-1]+ds_nz[k]);
	psi_Eyx[i][j][k] = bbx[i]*psi_Eyx[i][j][k] + aax[i]*(Hz[i-1][j][k]-Hz[i][j][k])/ds_x;
	Ey[i][j][k] = Ey[i][j][k] + dt/eo*(psi_Eyz[i][j][k] + psi_Eyx[i][j][k]);

	psi_Ezx[i][j][k] = bbx[i]*psi_Ezx[i][j][k] + aax[i]*(Hy[i][j][k]-Hy[i-1][j][k])/ds_x;
	psi_Ezy[i][j][k] = bby[j]*psi_Ezy[i][j][k] + aay[j]*(Hx[i][j-1][k]-Hx[i][j][k])/ds_y;
...
void cpml_H_field_update(int i, int j, int k)
{
	...
	Hx[i][j][k] = Hx[i][j][k] - tus*((Ez[i][j+1][k]-Ez[i][j][k])/ds_y/ffy[j] - (Ey[i][j][k+1]-Ey[i][j][k])/ds_nz[k]/ffz[k]);
	Hy[i][j][k] = Hy[i][j][k] - tus*((Ex[i][j][k+1]-Ex[i][j][k])/ds_nz[k]/ffz[k] - (Ez[i+1][j][k]-Ez[i][j][k])/ds_x/ffx[i]);
	Hz[i][j][k] = Hz[i][j][k] - tus*((Ey[i+1][j][k]-Ey[i][j][k])/ds_x/ffx[i] - (Ex[i][j+1][k]-Ex[i][j][k])/ds_y/ffy[j]);

	psi_Hxy[i][j][k] = hhy[j]*psi_Hxy[i][j][k] + ggy[j]*(Ez[i][j][k]-Ez[i][j+1][k])/ds_y;
	psi_Hxz[i][j][k] = hhz[k]*psi_Hxz[i][j][k] + ggz[k]*(Ey[i][j][k+1]-Ey[i][j][k])/ds_nz[k];
	Hx[i][j][k] = Hx[i][j][k] + tus*(psi_Hxy[i][j][k] + psi_Hxz[i][j][k]);

	psi_Hyz[i][j][k] = hhz[k]*psi_Hyz[i][j][k] + ggz[k]*(Ex[i][j][k]-Ex[i][j][k+1])/ds_nz[k];
	psi_Hyx[i][j][k] = hhx[i]*psi_Hyx[i][j][k] + ggx[i]*(Ez[i+1][j][k]-Ez[i][j][k])/ds_x;
	Hy[i][j][k] = Hy[i][j][k] + tus*(psi_Hyx[i][j][k] + psi_Hyz[i][j][k]);

	psi_Hzx[i][j][k] = hhx[i]*psi_Hzx[i][j][k] + ggx[i]*(Ey[i][j][k]-Ey[i+1][j][k])/ds_x;
	psi_Hzy[i][j][k] = hhy[j]*psi_Hzy[i][j][k] + ggy[j]*(Ex[i][j+1][k]-Ex[i][j][k])/ds_y;
...
void Ez_parity_boundary_update()
{
	...
	if(xparity==1)  //xparity is for Hz_parity
	{
		...
		for(k=1;k<p_mygrid_nz;k++)
		{
			Ex[c_mygrid_nx][j][k]=Ex[c_mygrid_nx-1][j][k];
			Ey[c_mygrid_nx][j][k]=0.0;
			Ez[c_mygrid_nx][j][k]=0.0;
		}
	}
	...
	if(yparity==1)
	{
		...
		for(i=1;i<p_mygrid_nx;i++)
		{
			Ey[i][c_mygrid_ny][k]=Ey[i][c_mygrid_ny-1][k];
			Ex[i][c_mygrid_ny][k]=0.0;
			Ez[i][c_mygrid_ny][k]=0.0;
		}
	}
	...
	if(zparity==1)
	{
		...
		for(j=1;j<p_mygrid_ny;j++)
		{
			Ez[i][j][c_mygrid_nz]=Ez[i][j][c_mygrid_nz-1];
			Ex[i][j][c_mygrid_nz]=0.0;
			Ey[i][j][c_mygrid_nz]=0.0;
		}
	}
...
void Ez_parity_iboundary_update()
{
	...
	if(xparity==1)  //xparity is for Hz_parity
	{
		...
		for(k=1;k<p_mygrid_nz;k++)
		{
			iEx[c_mygrid_nx][j][k]=iEx[c_mygrid_nx-1][j][k];
			iEy[c_mygrid_nx][j][k]=0.0;
			iEz[c_mygrid_nx][j][k]=0.0;
		}
	}
	...
	if(yparity==1)
	{
		...
		for(i=1;i<p_mygrid_nx;i++)
		{
			iEy[i][c_mygrid_ny][k]=iEy[i][c_mygrid_ny-1][k];
			iEx[i][c_mygrid_ny][k]=0.0;
			iEz[i][c_mygrid_ny][k]=0.0;
		}
	}
	...
	if(zparity==1)
	{
		...
		for(j=1;j<p_mygrid_ny;j++)
		{
			iEz[i][j][c_mygrid_nz]=iEz[i][j][c_mygrid_nz-1];
			iEx[i][j][c_mygrid_nz]=0.0;
			iEy[i][j][c_mygrid_nz]=0.0;
		}
	}
...
void Hz_parity_boundary_update()
{
	...
	if(xparity==1)
	{
		...
		for(k=1;k<p_mygrid_nz;k++)
		{
			Hy[c_mygrid_nx][j][k]=Hy[c_mygrid_nx-1][j][k];
			Hz[c_mygrid_nx][j][k]=Hz[c_mygrid_nx-1][j][k];
			Hx[c_mygrid_nx][j][k]=0.0;
		}
	}
	if(xparity==-1)
	{
	...
	if(yparity==1)
	{
		...
		for(i=1;i<p_mygrid_nx;i++)
		{
			Hz[i][c_mygrid_ny][k]=Hz[i][c_mygrid_ny-1][k];
			Hx[i][c_mygrid_ny][k]=Hx[i][c_mygrid_ny-1][k];
			Hy[i][c_mygrid_ny][k]=0.0;
		}
	}
	if(yparity==-1)
	{
	...
	if(zparity==1)
	{
		...
		for(j=1;j<p_mygrid_ny;j++)
		{
			Hx[i][j][c_mygrid_nz]=Hx[i][j][c_mygrid_nz-1];
			Hy[i][j][c_mygrid_nz]=Hy[i][j][c_mygrid_nz-1];
			Hz[i][j][c_mygrid_nz]=0.0;
		}
	}
	if(zparity==-1)
	{
...
void Hz_parity_iboundary_update()
{
	...
	if(xparity==1)
	{
		...
		for(k=1;k<p_mygrid_nz;k++)
		{
			iHy[c_mygrid_nx][j][k]=iHy[c_mygrid_nx-1][j][k];
			iHz[c_mygrid_nx][j][k]=iHz[c_mygrid_nx-1][j][k];
			iHx[c_mygrid_nx][j][k]=0.0;
		}
	}
	if(xparity==-1)
	{
	...
	if(yparity==1)
	{
		...
		for(i=1;i<p_mygrid_nx;i++)
		{
			iHz[i][c_mygrid_ny][k]=iHz[i][c_mygrid_ny-1][k];
			iHx[i][c_mygrid_ny][k]=iHx[i][c_mygrid_ny-1][k];
			iHy[i][c_mygrid_ny][k]=0.0;
		}
	}
	if(yparity==-1)
	{
	...
	if(zparity==1)
	{
		...
		for(j=1;j<p_mygrid_ny;j++)
		{
			iHx[i][j][c_mygrid_nz]=iHx[i][j][c_mygrid_nz-1];
			iHy[i][j][c_mygrid_nz]=iHy[i][j][c_mygrid_nz-1];
			iHz[i][j][c_mygrid_nz]=0.0;
		}
	}
	if(zparity==-1)
	{
...
void E_field_periodic_boundary_update_x()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ex[0][j][k]=Ex[mygrid_nx][j][k]*cos_ph+iEx[mygrid_nx][j][k]*sin_ph;
		Ey[mygrid_nx+1][j][k]=Ey[1][j][k]*cos_ph-iEy[1][j][k]*sin_ph;
		Ez[mygrid_nx+1][j][k]=Ez[1][j][k]*cos_ph-iEz[1][j][k]*sin_ph;

		iEx[0][j][k]=-Ex[mygrid_nx][j][k]*sin_ph+iEx[mygrid_nx][j][k]*cos_ph;
		iEy[mygrid_nx+1][j][k]=Ey[1][j][k]*sin_ph+iEy[1][j][k]*cos_ph;
		iEz[mygrid_nx+1][j][k]=Ez[1][j][k]*sin_ph+iEz[1][j][k]*cos_ph;
	}
}
...
void E_field_periodic_boundary_update_y()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ey[i][0][k]=Ey[i][mygrid_ny][k]*cos_ph+iEy[i][mygrid_ny][k]*sin_ph;
		Ex[i][mygrid_ny+1][k]=Ex[i][1][k]*cos_ph-iEx[i][1][k]*sin_ph;
		Ez[i][mygrid_ny+1][k]=Ez[i][1][k]*cos_ph-iEz[i][1][k]*sin_ph;

		iEy[i][0][k]=-Ey[i][mygrid_ny][k]*sin_ph+iEy[i][mygrid_ny][k]*cos_ph;
		iEx[i][mygrid_ny+1][k]=Ex[i][1][k]*sin_ph+iEx[i][1][k]*cos_ph;
		iEz[i][mygrid_ny+1][k]=Ez[i][1][k]*sin_ph+iEz[i][1][k]*cos_ph;
	}
}
...
void E_field_periodic_boundary_update_Ywall() ////// for triangular lattice 
{
	...
	for(i=1;i<p_mygrid_nx-Uxh;i++)
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ey[i][0][k]=Ey[i+Uxh][mygrid_ny][k]*cos_ph1+iEy[i+Uxh][mygrid_ny][k]*sin_ph1;
		Ex[i+Uxh][mygrid_ny+1][k]=Ex[i][1][k]*cos_ph1-iEx[i][1][k]*sin_ph1;
		Ez[i+Uxh][mygrid_ny+1][k]=Ez[i][1][k]*cos_ph1-iEz[i][1][k]*sin_ph1;

		iEy[i][0][k]=-Ey[i+Uxh][mygrid_ny][k]*sin_ph1+iEy[i+Uxh][mygrid_ny][k]*cos_ph1;
		iEx[i+Uxh][mygrid_ny+1][k]=Ex[i][1][k]*sin_ph1+iEx[i][1][k]*cos_ph1;
		iEz[i+Uxh][mygrid_ny+1][k]=Ez[i][1][k]*sin_ph1+iEz[i][1][k]*cos_ph1;
	}

	for(i=p_mygrid_nx-Uxh;i<p_mygrid_nx;i++)
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ey[i][0][k]=Ey[i-Uxh][mygrid_ny][k]*cos_ph2+iEy[i-Uxh][mygrid_ny][k]*sin_ph2;
		Ex[i-Uxh][mygrid_ny+1][k]=Ex[i][1][k]*cos_ph2-iEx[i][1][k]*sin_ph2;
		Ez[i-Uxh][mygrid_ny+1][k]=Ez[i][1][k]*cos_ph2-iEz[i][1][k]*sin_ph2;

		iEy[i][0][k]=-Ey[i-Uxh][mygrid_ny][k]*sin_ph2+iEy[i-Uxh][mygrid_ny][k]*cos_ph2;
		iEx[i-Uxh][mygrid_ny+1][k]=Ex[i][1][k]*sin_ph2+iEx[i][1][k]*cos_ph2;
		iEz[i-Uxh][mygrid_ny+1][k]=Ez[i][1][k]*sin_ph2+iEz[i][1][k]*cos_ph2;
	}
}
...
void H_field_periodic_boundary_update_x()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hx[mygrid_nx+1][j][k]=Hx[1][j][k]*cos_ph-iHx[1][j][k]*sin_ph;
		Hy[0][j][k]=Hy[mygrid_nx][j][k]*cos_ph+iHy[mygrid_nx][j][k]*sin_ph;
		Hz[0][j][k]=Hz[mygrid_nx][j][k]*cos_ph+iHz[mygrid_nx][j][k]*sin_ph;

		iHx[mygrid_nx+1][j][k]=Hx[1][j][k]*sin_ph+iHx[1][j][k]*cos_ph;
		iHy[0][j][k]=-Hy[mygrid_nx][j][k]*sin_ph+iHy[mygrid_nx][j][k]*cos_ph;
		iHz[0][j][k]=-Hz[mygrid_nx][j][k]*sin_ph+iHz[mygrid_nx][j][k]*cos_ph;
	}
}
...
void H_field_periodic_boundary_update_y()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hy[i][mygrid_ny+1][k]=Hy[i][1][k]*cos_ph-iHy[i][1][k]*sin_ph;
		Hx[i][0][k]=Hx[i][mygrid_ny][k]*cos_ph+iHx[i][mygrid_ny][k]*sin_ph;
		Hz[i][0][k]=Hz[i][mygrid_ny][k]*cos_ph+iHz[i][mygrid_ny][k]*sin_ph;

		iHy[i][mygrid_ny+1][k]=Hy[i][1][k]*sin_ph+iHy[i][1][k]*cos_ph;
		iHx[i][0][k]=-Hx[i][mygrid_ny][k]*sin_ph+iHx[i][mygrid_ny][k]*cos_ph;
		iHz[i][0][k]=-Hz[i][mygrid_ny][k]*sin_ph+iHz[i][mygrid_ny][k]*cos_ph;
	}
}
...
void H_field_periodic_boundary_update_Ywall()  ///// for triangular lattice
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hy[i+Uxh][mygrid_ny+1][k]=Hy[i][1][k]*cos_ph1-iHy[i][1][k]*sin_ph1;
		Hx[i][0][k]=Hx[i+Uxh][mygrid_ny][k]*cos_ph1+iHx[i+Uxh][mygrid_ny][k]*sin_ph1;
		Hz[i][0][k]=Hz[i+Uxh][mygrid_ny][k]*cos_ph1+iHz[i+Uxh][mygrid_ny][k]*sin_ph1;

		iHy[i+Uxh][mygrid_ny+1][k]=Hy[i][1][k]*sin_ph1+iHy[i][1][k]*cos_ph1;
		iHx[i][0][k]=-Hx[i+Uxh][mygrid_ny][k]*sin_ph1+iHx[i+Uxh][mygrid_ny][k]*cos_ph1;
		iHz[i][0][k]=-Hz[i+Uxh][mygrid_ny][k]*sin_ph1+iHz[i+Uxh][mygrid_ny][k]*cos_ph1;
	}

	for(i=p_mygrid_nx-Uxh;i<p_mygrid_nx;i++)
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hy[i-Uxh][mygrid_ny+1][k]=Hy[i][1][k]*cos_ph2-iHy[i][1][k]*sin_ph2;
		Hx[i][0][k]=Hx[i-Uxh][mygrid_ny][k]*cos_ph2+iHx[i-Uxh][mygrid_ny][k]*sin_ph2;
		Hz[i][0][k]=Hz[i-Uxh][mygrid_ny][k]*cos_ph2+iHz[i-Uxh][mygrid_ny][k]*sin_ph2;

		iHy[i-Uxh][mygrid_ny+1][k]=Hy[i][1][k]*sin_ph2+iHy[i][1][k]*cos_ph2;
		iHx[i][0][k]=-Hx[i-Uxh][mygrid_ny][k]*sin_ph2+iHx[i-Uxh][mygrid_ny][k]*cos_ph2;
		iHz[i][0][k]=-Hz[i-Uxh][mygrid_ny][k]*sin_ph2+iHz[i-Uxh][mygrid_ny][k]*cos_ph2;
	}
}
...
void E_field_Gamma_boundary_update_x()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ex[0][j][k]=Ex[mygrid_nx][j][k];
		Ey[mygrid_nx+1][j][k]=Ey[1][j][k];
		Ez[mygrid_nx+1][j][k]=Ez[1][j][k];
	}
}
...
void E_field_Gamma_boundary_update_y()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ey[i][0][k]=Ey[i][mygrid_ny][k];
		Ex[i][mygrid_ny+1][k]=Ex[i][1][k];
		Ez[i][mygrid_ny+1][k]=Ez[i][1][k];
	}
}
...
void E_field_Gamma_boundary_update_Ywall() ///// for triangular lattice
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Ey[i][0][k]=Ey[i+Uxh][mygrid_ny][k];
		Ex[i+Uxh][mygrid_ny+1][k]=Ex[i][1][k];
		Ez[i+Uxh][mygrid_ny+1][k]=Ez[i][1][k];
	}
...
void H_field_Gamma_boundary_update_x()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hx[mygrid_nx+1][j][k]=Hx[1][j][k];
		Hy[0][j][k]=Hy[mygrid_nx][j][k];
		Hz[0][j][k]=Hz[mygrid_nx][j][k];
	}
}
...
void H_field_Gamma_boundary_update_y()
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hy[i][mygrid_ny+1][k]=Hy[i][1][k];
		Hx[i][0][k]=Hx[i][mygrid_ny][k];
		Hz[i][0][k]=Hz[i][mygrid_ny][k];
	}
}
...
void H_field_Gamma_boundary_update_Ywall() ///// for triangular lattice
{
	...
	for(k=1;k<p_mygrid_nz;k++)
	{
		Hy[i+Uxh][mygrid_ny+1][k]=Hy[i][1][k];
		Hx[i][0][k]=Hx[i+Uxh][mygrid_ny][k];
		Hz[i][0][k]=Hz[i+Uxh][mygrid_ny][k];
	}
